...
`run-cycle` が 1 周ごとに残す**行動の履歴（append-only）**。台帳（`challenge-ledger.md`）が「課題の現在状態」を表すのに対し、journal は「その状態にどう至ったか」を表す。
...
3. **作成した PR・ブランチの URL**: FR-22（run-cycle step 3）の「自律で PR を作成したら PR URL を必ずサイクルレポートに出す」の固定置き場。
