...
- **冪等**: 同じ課題を再取り込みしても二重登録しない。既存エントリの `fp`（**取得したての外部ソース本文**のフィンガープリント。算出は ingest-challenges が `scripts/ingest-fp.sh` で行う）が変われば**人間記入欄だけ**更新し、分類・ステータスは保持する（台帳の記入形式は `challenge-ledger.md` の記入例を参照）。
...
- **頻度**: 手動（`/ingest-challenges`）／`run-cycle` の観測ステップから自動のいずれでもよい。
