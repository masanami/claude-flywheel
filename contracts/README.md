...
- **append-only・原文保存の恒久記録は「当該操作の追記分だけ」を検証する**: `journal/index.jsonl`・`runs.jsonl` は既存行を書き換えない恒久記録、`challenge-archive.md` は「ステータス行以外は原文のまま」保存する履歴であり、いずれも契約導入前の不正な過去分が残っていることがある（過去分の正しさは正本が保証せず、「修復」は履歴・原文の書き換えになる）。全体検証だと過去分の違反で以後の全操作が恒久失敗するため、run-cycle 手順6 は `journal-index` に `--tail 1`（正本保証「1 周 1 行 append」に基づく）、`runs` に `--since-last-cycle-start`（1 周の追記行数が可変のため、当周の開始マーカーからの範囲指定）、`archive` に `--tail-entries <追記エントリ数>` を使う。**過去分は「検証して正常」ではなく「対象外」**（読み替えない）。範囲限定モードは同時に**追記の実在**も検証する（`--tail n`＝末尾が非空レコードで終わり n 件以上存在すること・`--tail-entries n`＝エントリが n 件以上存在すること。空行のみの追記・追記の消失を「前回分を検証して正常」と誤証明しない）。契約全体の検証（vendoring 先のパーサテスト・フィクスチャ）は従来どおり全体が対象。**台帳（`challenge-ledger.md`）だけは現在状態のファイル**（修復が正規の運用・修復実績あり）のため全体を検証する。
