...
| ingest-challenges | 取り込み元（`<!-- fp:... -->`） | 説明は**外部本文の要約（1 行）**（[#130](https://github.com/masanami/claude-flywheel/issues/130)）。ただし要約化より前に取り込まれたエントリとアーカイブでは**ブロック引用の複数行**になりうるため、**両方を受理する** | 同上 |
...
| 同 §説明欄 | **外部ソース由来のエントリの `説明` は外部本文の要約**（[#130](https://github.com/masanami/claude-flywheel/issues/130)）。書き手は新たに引用形を作らないが、**引用形の受理は続く**（既存台帳・アーカイブに残るため）。**消費側の実装は変更不要**——読み取り規則は変わっておらず、値が短くなるだけである |
