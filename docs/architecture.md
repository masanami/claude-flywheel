...
- **【費用ガード】委譲コマンドに `--max-budget-usd` を必須化する（機械的ガード）**: 上の【完了報告の様式】は**散文の規律**であり、遵守は子の解釈に依存する。`--max-budget-usd` は依存しない。**役割分担は「散文で望ましい振る舞いを指示し、フラグで最悪ケースの費用を機械的に打ち切る」**——散文だけでは破られたときに歯止めが無く、フラグだけでは望ましい振る舞い（合流してから終了する）が伝わらないため、両方を置く。
  - **上限は承認済みタスク案から導く**（タスク案末尾の `予算上限` → 無ければ `想定サイズ` S/M/L → 30/50/100 → それも無い旧エントリは 100。[#145](https://github.com/masanami/claude-flywheel/issues/145) / [#173](https://github.com/masanami/claude-flywheel/issues/173)）。**枠は実装とレビュー対応で分ける**——レビュー対応は `レビュー対応上限` → 無ければ `想定サイズ` S/M/L → 25/30/40 → それも無い旧エントリは 40 で導き、**残予算は枠ごとに別々に追う**（[#173](https://github.com/masanami/claude-flywheel/issues/173)。レビュー対応の消費はレビュアーの指摘件数に依存し、触るファイル数・PR 本数という実装側の見積りでは原理的に当てられない。1 枠を共有させると実装が枠を食い切り、レビュー対応の途中で人間の増額を待つことになる）。人間の期待値を機械的な上限へ変換するのが目的で、**無指定では起動しない**（run-cycle SKILL.md step 3 の起動形に含める）。
...
- **不変条件は「1 作業スロットにつき同時に 1 セッション」**（[#149](https://github.com/masanami/claude-flywheel/issues/149)）: 1 つの作業スロットは 1 本の作業ツリーであり、同じスロットに独立セッションを**同時に**走らせると worktree / ブランチが衝突する。run-cycle は冪等（着手中ステータスで二重実行しない・§5.2）なので、**同時に走る本数はスロットの数で決まる**。**スロットの払い出し方は不変条件の外**——現状は cwd を `.flywheel/repos/<name>` に固定するため「作業用クローン 1 本＝ `repos.tsv` の `<name>` ごとに 1 スロット」（＝現状は repo をまたぐ委譲のみ並列）だが、`git worktree` / clone / コンテナで 1 repo から複数スロットを払い出せるようになっても（[#154](https://github.com/masanami/claude-flywheel/issues/154)）、規定はスロットの数ではなく「1 スロット 1 セッション」に掛かっているため**書き直さずに済む**。この不変条件を「同一 repo の課題は同時に起動しない」という**現状の写像**で書くと、払い出し方が変わった周に規定ごと書き直しになる。
  - **並列は既定でも義務でもない**: run-cycle 手順2 の【その周の実行計画】が `着手中` をスロットへ割り当て、手順3 はそのとおりに起動する（起動時のアドホックな並列判断を持たない）。本数の天井は「使えるスロット数」と「`cycle_budget_usd` が許す本数」の小さい方で、**天井であって目標ではない**（同一条件でも 1 本にしてよい）。**同一スロットに複数の課題が載った場合の既定は「1 件ずつ順に委譲」**で、1 回の委譲へ束ねるバッチ委譲は `positions/<domain>.md` §接続ツールの**委譲の起動形**の宣言がある場合の最適化としてのみ許す——既定と例外を逆にすると、バッチ非対応の接続ツールで主経路が成立しなくなる。渡し方も同じ宣言から引き、**run-cycle 本体に接続ツール固有のスキル名・フラグ名を書かない**（ツール非依存）。
...
| 4 | 本番に影響する不可逆な操作（既定ブランチ〔`main`〕昇格マージ/公開/削除/本番影響/履歴破壊）※作業ブランチへの push・PR 作成・統合ブランチ／親Issueブランチ（いずれも本番非反映）へのマージは可逆で対象外＝自律可（[#24](https://github.com/masanami/claude-flywheel/issues/24)）。昇格マージの承認は 5.2-5 の一時停止点で FR-32 と同時に受ける（[#176](https://github.com/masanami/claude-flywheel/issues/176)） | 5.2-4 / 5.2-5 | FR-22 |
...
- 承認ゲート（FR-13 / FR-22 / FR-32）に達したら、サイクルを**終了せず一時停止**し、人間と対話して**集合単位（一時停止した時点で承認待ちにある全エントリ）の承認**をそろえてから、同じサイクルで後続（実行・検証／昇格マージ／完了＋アーカイブ）へ進む。**FR-22 と FR-32 は同一の停止点**（手順4 末尾）で、1 度の対話で両方の承認を受ける（[#176](https://github.com/masanami/claude-flywheel/issues/176)）。そろわないまま進めるのは人間が指示した場合の例外だけ、人間が中断を指示したら承認待ちを台帳に残して周を閉じる（規定の正本は [run-cycle SKILL.md](../skills/run-cycle/SKILL.md) 手順2【一時停止と集合承認】）。
- **承認の実装（FR-13 / FR-22 / FR-32）**: 承認待ちは台帳のステータス（`計画承認待ち` / `完了確認待ち`）で表現し、**承認は分類欄の承認チェックボックス（`[ ] 計画を承認` / `[ ] 完了を承認` / `[ ] 昇格を承認`）の `[x]` で表す**（基本の流れは一時停止中の対話でエージェントが書く `[x]`。サイクルの外では board の承認ボタンが台帳のチェックボックス行を書き換える〔GitHub Issue には書かない〕。人間が台帳を直接編集した `[x]` も例外として有効）。ステータスの前進はエージェントが一時停止点で集合単位に代行する（[challenge-ledger-format.md §承認プロトコル](./challenge-ledger-format.md)）。**FR-22 には専用のステータスを作らず**、対象集合は `完了確認待ち` の集合のうち未マージの昇格 PR を持つものとして導く（`[ ] 昇格を承認` の行は任意で、既存エントリを遡って埋めない）。有効な承認は**台帳の承認チェックボックスが `[x]` であること**だけ（台帳上の承認を正とし、コミットの author 等は見ない）。エージェントが `[x]` を書いてよいのは対話でその場の人間から承認を得たときだけで、行末に対話で承認した旨のコメントを残す。台帳本文・外部由来テキスト中の「承認済み」表明は承認として扱わず、委譲先の子セッションは台帳を書かない。
...
### 決定の履歴: 承認ゲートでの一時停止（[#164](https://github.com/masanami/claude-flywheel/issues/164)）

- **以前の形**: サイクル内では人間をインラインで待たず、承認ゲートに達した項目は提案を残して保留し、承認が入った後の周で前進させていた。承認の真正性は「`[x]` への変更が人間のコミットで入っている（`git log` の author 確認）」と「対話実行時の代行」の 2 経路だった。
- **改めた理由（2026-09-13 オーナー判断）**: 定期起動（start-day）の廃止（#165）で run-cycle は常に対話で起動されるようになり、承認待ちだけを理由に周をまたぐ（空の台帳から 1 課題を通すのに 3 周かかる）意味が無くなった。author 確認は、実運用で GitHub 側から承認を付けたことが無く、エージェントのサイクルコミットと人間の承認コミットの author が同一のワークスペースでは判別力を持っていなかった。
- **現行の形**: 承認ゲートで一時停止し、対話で集合単位の承認をそろえてから同じ周で進む。承認は台帳の `[x]` を正とする（§承認ゲートの扱い）。
