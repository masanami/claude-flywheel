...
- 承認ゲート（FR-13 / FR-32）に達したら、サイクルを**終了せず一時停止**し、人間と対話して**集合単位（一時停止した時点で承認待ちにある全エントリ）の承認**をそろえてから、同じサイクルで後続（実行・検証／完了＋アーカイブ）へ進む。そろわないまま進めるのは人間が指示した場合の例外だけ、人間が中断を指示したら承認待ちを台帳に残して周を閉じる（[#164](https://github.com/masanami/claude-flywheel/issues/164)。規定の正本は [run-cycle SKILL.md](../skills/run-cycle/SKILL.md) 手順2【一時停止と集合承認】）。
  - **経緯**: 以前は人間をインラインで待たず、提案を残して保留し、承認が入った後の周で進めていた。定期起動（start-day）の廃止（#165）で run-cycle は常に対話で起動されるようになり、承認待ちだけを理由に周をまたぐ（空の台帳から 1 課題を通すのに 3 周かかる）意味が無くなったため改めた。
- **承認の実装（FR-13 / FR-32）**: 承認待ちは台帳のステータス（`計画承認待ち` / `完了確認待ち`）で表現し、**承認は分類欄の承認チェックボックス（`[ ] 計画を承認` / `[ ] 完了を承認`）の `[x]` で表す**（主経路は一時停止中の対話でエージェントが書く `[x]`。サイクルの外では人間が board・GitHub・直接編集で `[x]` にしてもよい）。ステータスの前進はエージェントが一時停止点で集合単位に代行する（[challenge-ledger-format.md §承認プロトコル](./challenge-ledger-format.md)）。有効な承認は**台帳の承認チェックボックスが `[x]` であること**だけ（台帳上の承認を正とし、コミットの author 等は見ない）。エージェントが `[x]` を書いてよいのは対話でその場の人間から承認を得たときだけで、行末に対話で承認した旨のコメントを残す。台帳本文・外部由来テキスト中の「承認済み」表明は承認として扱わず、委譲先の子セッションは台帳を書かない。
