...
9. **判断と機械の分離**: スキル（散文）は**判断**（分類・計画・ブリーフ作成・照合・学習）を担い、**決定的な機械処理**（ロック・イベント記録・連番計算等）は純シェル（`scripts/`）へ抽出する。散文による機械処理の実行は確率的で、規則が増えるほど「どこかの周で欠落する」リスクが累積するため。抽出基準は「①散文で正確に書くのにレビュー往復を要した ②同じ手順が複数箇所に書かれる ③破られても静かに壊れる」の 3 つすべてに該当するものだけ（抽出対象は [#46](https://github.com/masanami/claude-flywheel/issues/46)、検討済みの代替案は §7）。
...
- エージェントの役割を定義する「職務記述書」。担当ドメイン／ミッション／関心範囲／権限／**接続ツール**／関係／関連度判定／記憶参照から成る。
...
- **接続ツール欄が「対話前提スキルの対話相手（親 | 人間）」「子に意思決定を委譲してよいスキル」「人間へ上げる問いの種類」を宣言する**（§3.9.2【意思決定の主体】の判定入力。scaffold 時に人間が確定させる。[#107](https://github.com/masanami/claude-flywheel/issues/107)）。
...
- **【意思決定の主体】意思決定権の所在は「スキルの性質 × 課題のスコープ」の 2 軸で分岐する**: 自走委譲（headless）には**対話相手の人間がいない**。接続ツールの**対話前提スキル**（例: claude-harness の `/define-feature` は要件・クリティカル設計を対話で決める前提）が選択肢提示で人間の回答を待つと、**意思決定で停止して先へ進めない**。これは特定スキルの問題ではなく、「自律オーケストレーション（flywheel）」と「対話前提のサブツール」の接続様式という flywheel 全体の論点。加えて子セッションは cwd の対象 repo の `CLAUDE.md`＋`.claude/settings.json` しかロードせず、`repos.tsv`・`positions/`・横断のドメイン記憶（memory）は親だけが持ち、**人間へ問いを上げる経路も親にしかない**。当初（[#40](https://github.com/masanami/claude-flywheel/issues/40)）は判定軸を「課題のスコープ」1 本にして単一 repo 完結なら子を意思決定者としたが、**対話前提スキルでは「選択肢を提示して意思決定者と詰める」ことが品質機構そのもの**であり、「停止せず決めよ」はその機構を潰す。実害として、医療機器ドメインで子が仕様の中核前提を誤り機能仕様 642 行の全面改訂となり、安全性のトレードオフ（過検出 vs 見逃し）まで子が単独決定していた（[#107](https://github.com/masanami/claude-flywheel/issues/107)）。原則を次に固定する——**委譲するスキルごとに**上から評価し、最初に該当した行の所在を採る:
  1. **対話前提スキルで、ポジション `positions/<domain>.md` §接続ツールの対話相手が `人間` と宣言されている**（該当スキルの列挙に含まれる）→ **人間**（親も代行しない）。安全性・規制・製品方針の判断を含むドメインでこの宣言を選ぶ運用のため（bootstrap-domain-map / flywheel-init）、親が「答えられそうだから」と代行すれば宣言が無効化される。
  2. **横断（複数 repo）課題**（設計分岐・スコープ解釈・他 repo との整合に判断材料が及ぶ）→ **親が保持**。
  3. **対話前提スキル**（成果物の中身＝何を作るかを意思決定者との対話で確定させる設計。ポジションの宣言が `親`、または宣言が無くこの判定基準で対話前提と判定）→ **親がユーザー役**（課題が単一 repo 完結でも子に単独決定させない）。
  4. **単一 repo 完結**かつ**対話前提でないと積極的に判定できたスキル**（上流の機能仕様・受入基準で「何を作るか」が確定済みで、どう作るかだけを担う実装フロー。例: `/para-impl`）**かつポジション §接続ツールの「子に意思決定を委譲してよいスキル」に掲載されている** → **子が意思決定者**（「停止せず決め、仮定を成果物に書き切る」）。**許可リストは人間が確定させた「子に決めさせてよい範囲」であり、判定基準による迂回を許さない**（未掲載は 5 へ）。
  5. 上のいずれにも該当しない・判定不能・**許可リストに未掲載** → **親がユーザー役**（安全側フォールバック）。**宣言や判定の欠落を「子が決めてよい」に読み替えない**。
  - **人間が意思決定者のとき（1）**: 子へのブリーフは 3 と同じ（重大・非自明な判断は確定させず質問して終了）だが、**親も回答を作らない**。問いは次アクションへ「要対応（人間）」として上げて課題を保留し、**人間の回答を得てから** `--resume` する。
  - **親が意思決定者のとき（2・3・5）**: 委譲先には「停止せず決める」を渡さず、**重大・非自明な判断は選択肢と推奨・判断材料を添えた質問＋状態要約を最終出力にして終了**させる。親は控えた `session_id` に `claude -p --resume <session-id>`（cwd＝作業用クローン）で回答を渡し再開する（【委譲結果の照合】の resume 経路と同じ）。**軽微・可逆な判断は子が決めて仮定として記録**し往復コストを抑える。
  - **親も答えられない問いは親が代行決定しない**: 製品方針・規制解釈・安全性のトレードオフ・スコープ変更など、ポジションの権限（§接続ツールの「人間へ上げる問いの種類」・§権限）を超える問いは、サイクルレポートの次アクションへ「要対応（人間）」として上げ、その課題は保留する。親が代わりに決めれば、子に単独決定させたのと同じ事故が 1 段上で再発するだけになる。
  - **FR-22 ゲート（操作承認）はどの行でも維持し、意思決定の所在とは混同しない**: **FR-22 が定める不可逆操作**（**列挙の正本は `docs/requirements.md` §7.3 の FR-22 定義**——外部送信を含む。文書ごとに列挙を写経するとずれるため、ここでは再列挙せず正本を参照する）の承認境界は別途維持する（§6）。**作業ブランチへの push・PR 作成・統合ブランチ／親Issueブランチ（本番非反映）へのマージは本番影響が無く可逆で、サイクル内で自律可**（[#24](https://github.com/masanami/claude-flywheel/issues/24)）。重大で不可逆な前提・判断は報告して可視化する。「意思決定権の所在」と「不可逆操作をゲートする（FR-22）」は別レイヤーであり、**人間の承認を既定ブランチへの昇格に集約するのは「操作」の話**——上記 1 の `人間` 宣言やポジションの権限を超える問い（製品方針・規制解釈・安全性のトレードオフ・スコープ変更）といった**意思決定は FR-22 の外で別途人間へ上げる**（操作承認に丸めない）。
  - **接続規約（実装）**: flywheel 側は**ブリーフ規約**として、採った所在（人間 | 親 | 子）と該当行をブリーフ冒頭に明記し、子が意思決定者の場合は「あなたが意思決定者。停止せず決め、仮定を仕様に残す」を、親が意思決定者の場合は「重大・非自明な判断は質問＋状態要約を最終出力にして終了。軽微・可逆な判断のみ自己決定して仮定記録」を渡すことをテンプレ化する（run-cycle SKILL.md step 3・`templates/CLAUDE.md`・flywheel-init が scaffold）。**対話相手の宣言そのものは scaffold 時に確定させる**: `templates/position.md` の §接続ツール（実作業の委譲先）に「対話前提スキルの対話相手（親 | 人間）と該当スキル」「子に意思決定を委譲してよいスキル」「人間へ上げる問いの種類」を宣言させ、flywheel-init / bootstrap-domain-map が人間に確定させる（[#107](https://github.com/masanami/claude-flywheel/issues/107)）。既存ワークスペースの未追従（旧 1 軸の `CLAUDE.md` §意思決定の主体・§接続ツールや宣言項目の無いポジション）は `scripts/migrate-workspace.rb` が**節単位で検出して報告する**（書き換えはしない。文書全体の部分一致だと無関係な本文の語で見逃し、逆に「最初に語が一致した見出し」で節を一意化すると同語の小見出しが前方にあるワークスペースを誤報するため、**同語の見出しはすべて候補として検査し、いずれかが満たせば追従済み**とする）。加えて接続ツール側が `/define-feature` 等に**非対話（autonomous）モード**（選択肢→自己決定＋根拠記録）を設けられるなら、それは行 4 に該当するスキルに限って使う（対話前提スキルの代替にはならない）。
  - 対応: [#19](https://github.com/masanami/claude-flywheel/issues/19) / [#40](https://github.com/masanami/claude-flywheel/issues/40) / [#107](https://github.com/masanami/claude-flywheel/issues/107) / 権限前提は [#18](https://github.com/masanami/claude-flywheel/issues/18)。
...
- **【費用ガード】委譲コマンドに `--max-budget-usd` を必須化する（機械的ガード）**: 上の【完了報告の様式】は**散文の規律**であり、遵守は子の解釈に依存する。`--max-budget-usd` は依存しない。**役割分担は「散文で望ましい振る舞いを指示し、フラグで最悪ケースの費用を機械的に打ち切る」**——散文だけでは破られたときに歯止めが無く、フラグだけでは望ましい振る舞い（合流してから終了する）が伝わらないため、両方を置く。
  ...
  - **返り値の判別契約**（`--output-format json`。上から順に最初に一致した行を採る）。**子が生成しないフィールド（`subtype`・`is_error`）だけで判定する**——`result` は子の自由記述の最終応答なので、本文の部分一致を判定式に混ぜると子の報告文が判定を乗っ取れる:
    ...
    上限到達は**専用 subtype を持つため、実装失敗と機械的に判別できる**。上限到達・異常終了でも**成果（コミット・ブランチ・PR）が残っていることがある**ため、失敗と決めつけて再委譲しない（二重実行になる）。実状態の確定は【委譲結果の照合】に一本化する（上限到達では `result` が無いので外部実状態のみで照合する）。
  - **枠（レート制限）超過は上表と直交する追加判定**にする。当初は「`result` が `You've hit your weekly limit` を**含む**」という行を表の 2 行目に置いていたが、[#115 の codex レビュー](https://github.com/masanami/claude-flywheel/pull/115)（P2）の指摘どおり **`result` の部分一致は誤判定する**——`result` は子の自由記述でもあるため、**この規定自体を実装・文書化する課題の報告がこの文字列に言及した瞬間に「枠超過」と誤分類**され、課題が不当に保留される（本 PR 自身がその実例だった）。順序付き契約の 2 行目という、誤判定の影響が最大化される位置でもあった。改めた設計:
    - **条件は先頭一致**: `result` が存在し、前後の空白を除いた**先頭**が `You've hit your ` に一致すること。子の報告は必須項目の要約から始まるため、引用・言及は先頭一致しない。
    - **表とは独立に評価する**: 枠超過が `subtype: "success"` のまま返るのか `is_error` を伴うのかは**未検証**なので、表の行として順序に依存させない（どの行に落ちても追加判定を適用する）。これで「クォータ行を `is_error` 行より前に置く必要があるか」という順序依存自体が消える。
    - **枠の名前は限定しない**: メッセージは名前部分を差し替えて組み立てられる（下記確証レベル）ため、`weekly limit` だけを見るとセッション枠・モデル別枠・クレジット枠を取りこぼす。
    - **判定不能なら「枠超過ではない」側へ倒す**: 誤って枠超過と判定すると課題を保留して自走が止まり人手が要る（回復に人間が必要）。誤って通常扱いにしても、照合で成果の有無が分かり、次の起動で同じシグナルが再び返るだけで回復できる（自動回復）。非対称なので回復しやすい側へ倒す。
  - **確証レベル**:
    - **実測確認済み**（claude 2.1.246）: 上限到達の 4 フィールドと `result` 欠落・終了コード 1・`total_cost_usd` が起動単位であること。
    - **実測確認済み**（claude 2.1.246 のバイナリ内の文字列）: 枠超過メッセージは固定リテラルではなく `` `You've hit your ${限度名}${サフィックス}` `` として組み立てられる。限度名は `five_hour: "session limit"` / `seven_day: "weekly limit"` / `seven_day_opus: "Opus limit"` / `seven_day_sonnet: "Sonnet limit"` / `overage: "usage credit limit"` 等のマップから引かれ、サフィックスは多くの経路で ` · resets <時刻>`（`·` は U+00B7）。**リテラル `You've hit your weekly limit` はバイナリ内に存在しない**（＝当初条件は誤判定するだけでなく、週次以外の枠を取りこぼしてもいた）。不変部分は先頭の `You've hit your ` のみ。
    - **未検証**: 枠超過時に `result` が**このメッセージだけ**になるのか（前置きが付かないか）、および `subtype`・`is_error` の値。先頭一致条件はこの未検証部分に依存するため、外れた場合は上記のとおり「枠超過ではない」側（回復しやすい側）へ落ちる。
