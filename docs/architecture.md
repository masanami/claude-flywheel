...
- **起動方式は `claude -p` に一本化（YAGNI）**:
  ...
  - 権限は **FR-22 の承認境界**に従う: ローカル編集・commit・**topic push・PR（draft 含む）作成・統合／親Issueブランチ（本番非反映）へのマージは自律可**（本番影響が無く可逆）、**既定ブランチ〔`main`〕への昇格マージ / 本番影響 / 削除 / 履歴破壊が承認ゲート対象**（§5.2 step 4・§6・[#24](https://github.com/masanami/claude-flywheel/issues/24)）。判定軸は「本番への影響」と「可逆性」。起動例の `--permission-mode acceptEdits` は編集の自動承認で、force-push 等の不可逆操作は対象 repo の `settings.json`（deny）でゲートする。
...
- **【意思決定の主体】委譲先エージェントが『ユーザー＝意思決定者』役を担う**: 自走委譲（headless）には**対話相手の人間がいない**。接続ツールの**対話前提スキル**（例: claude-harness の `/define-feature` は要件・クリティカル設計を対話で決める前提）が選択肢提示で人間の回答を待つと、**意思決定で停止して先へ進めない**。これは特定スキルの問題ではなく、「自律オーケストレーション（flywheel）」と「対話前提のサブツール」の接続様式という flywheel 全体の論点。原則を次に固定する:
  ...
  - **FR-22 ゲートは維持**: 本番に影響する不可逆な操作（既定ブランチ〔`main`〕への昇格マージ / 本番影響 / 削除 / 履歴破壊）の承認境界は別途維持する（§6）。**topic push・PR 作成・統合／親Issueブランチへのマージは本番影響が無く可逆で、サイクル内で自律可**（[#24](https://github.com/masanami/claude-flywheel/issues/24)）。重大で不可逆な前提・判断は報告して可視化する。「停止しない（意思決定移譲）」と「不可逆操作をゲートする（FR-22）」は別レイヤー。
...
| 4 | 本番に影響する不可逆な操作（既定ブランチ〔`main`〕昇格マージ/公開/削除/本番影響/履歴破壊）※topic push・PR 作成・統合／親Issueブランチ（本番非反映）へのマージは可逆で対象外＝自律可（[#24](https://github.com/masanami/claude-flywheel/issues/24)） | 5.2-4 | FR-22 |
