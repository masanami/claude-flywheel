...
- **既定は 1 エージェント 1 ポジション**。拡張する場合の分割軸は **サブ領域**を基本とする（例: インフラ・セキュリティ横断 → 「ネットワーク」「IAM」「シークレット」）。機能（開発/運用）は通常エージェント単位で既に分かれているため、エージェント内をさらに機能で割らない。分割軸はエージェントごとに 1 つへ統一する。
