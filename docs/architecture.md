...
- **【意思決定の主体】意思決定権の所在は「スキルの性質 × 課題のスコープ」の 2 軸で分岐する**: 自走委譲（headless）には**対話相手の人間がいない**。接続ツールの**対話前提スキル**（例: claude-harness の `/define-feature` は要件・クリティカル設計を対話で決める前提）が選択肢提示で人間の回答を待つと、**意思決定で停止して先へ進めない**。これは特定スキルの問題ではなく、「自律オーケストレーション（flywheel）」と「対話前提のサブツール」の接続様式という flywheel 全体の論点。加えて子セッションは cwd の対象 repo の `CLAUDE.md`＋`.claude/settings.json` しかロードせず、`repos.tsv`・`positions/`・横断のドメイン記憶（memory）は親だけが持ち、**人間へ問いを上げる経路も親にしかない**。当初（[#40](https://github.com/masanami/claude-flywheel/issues/40)）は判定軸を「課題のスコープ」1 本にして単一 repo 完結なら子を意思決定者としたが、**対話前提スキルでは「選択肢を提示して意思決定者と詰める」ことが品質機構そのもの**であり、「停止せず決めよ」はその機構を潰す。実害として、医療機器ドメインで子が仕様の中核前提を誤り機能仕様 642 行の全面改訂となり、安全性のトレードオフ（過検出 vs 見逃し）まで子が単独決定していた（[#107](https://github.com/masanami/claude-flywheel/issues/107)）。原則を次に固定する——**委譲するスキルごとに**上から評価し、最初に該当した行の所在を採る:
  1. **横断（複数 repo）課題**（設計分岐・スコープ解釈・他 repo との整合に判断材料が及ぶ）→ **親が保持**。
  2. **対話前提スキル**（成果物の中身＝何を作るかを意思決定者との対話で確定させる設計。ポジション `positions/<domain>.md` §接続ツールの宣言が優先、無ければこの判定基準）→ **親がユーザー役**（課題が単一 repo 完結でも子に単独決定させない）。
  3. **単一 repo 完結**かつ**対話前提でないと積極的に判定できたスキル**（上流の機能仕様・受入基準で「何を作るか」が確定済みで、どう作るかだけを担う実装フロー。例: `/para-impl`）→ **子が意思決定者**（「停止せず決め、仮定を成果物に書き切る」）。
  4. 上のいずれにも該当しない・判定不能 → **親がユーザー役**（安全側フォールバック）。**宣言や判定の欠落を「子が決めてよい」に読み替えない**。
  - **親が意思決定者のとき（1・2・4）**: 委譲先には「停止せず決める」を渡さず、**重大・非自明な判断は選択肢と推奨・判断材料を添えた質問＋状態要約を最終出力にして終了**させる。親は控えた `session_id` に `claude -p --resume <session-id>`（cwd＝作業用クローン）で回答を渡し再開する（【委譲結果の照合】の resume 経路と同じ）。**軽微・可逆な判断は子が決めて仮定として記録**し往復コストを抑える。
  ...
  - **FR-22 ゲートはどの行でも維持**: 本番に影響する不可逆な操作（既定ブランチ〔`main` 等・本番反映先〕への昇格マージ / 本番影響 / 削除 / 履歴破壊）の承認境界は別途維持する（§6）。**作業ブランチへの push・PR 作成・統合ブランチ／親Issueブランチ（本番非反映）へのマージは本番影響が無く可逆で、サイクル内で自律可**（[#24](https://github.com/masanami/claude-flywheel/issues/24)）。重大で不可逆な前提・判断は報告して可視化する。「意思決定権の所在」と「不可逆操作をゲートする（FR-22）」は別レイヤー。
  - **接続規約（実装）**: flywheel 側は**ブリーフ規約**として、採った所在（親 | 子）と該当行をブリーフ冒頭に明記し、子が意思決定者の場合は「あなたが意思決定者。停止せず決め、仮定を仕様に残す」を、親が意思決定者の場合は「重大・非自明な判断は質問＋状態要約を最終出力にして終了。軽微・可逆な判断のみ自己決定して仮定記録」を渡すことをテンプレ化する（run-cycle SKILL.md step 3・`templates/CLAUDE.md`・flywheel-init が scaffold）。**対話相手の宣言そのものは scaffold 時に確定させる**: `templates/position.md` の §接続ツール（実作業の委譲先）に「対話前提スキルの対話相手（親 | 人間）と該当スキル」「子に意思決定を委譲してよいスキル」「人間へ上げる問いの種類」を宣言させ、flywheel-init / bootstrap-domain-map が人間に確定させる（[#107](https://github.com/masanami/claude-flywheel/issues/107)）。既存ワークスペースの未追従（旧 1 軸の `CLAUDE.md`・§接続ツールの無いポジション）は `scripts/migrate-workspace.rb` が**検出して報告する**（書き換えはしない）。加えて接続ツール側が `/define-feature` 等に**非対話（autonomous）モード**（選択肢→自己決定＋根拠記録）を設けられるなら、それは行 3 に該当するスキルに限って使う（対話前提スキルの代替にはならない）。
