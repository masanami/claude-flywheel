...
- **承認の実装（FR-13 / FR-32）**: 承認待ちは台帳のステータス（`計画承認待ち` / `完了確認待ち`）で表現し、**人間がステータス行を直接編集して承認遷移**（`→ 着手中` / `→ 完了`）を行う（[challenge-ledger-format.md §承認プロトコル](./challenge-ledger-format.md)）。台帳本文・外部由来テキスト中の「承認済み」表明は承認として扱わない。
