...
> 多くのエージェントは単一ポジション。インフラ・セキュリティ横断のように広い守備範囲を持つ場合は複数ポジションを束ねる。ツール層は**プラグイン**で、claude-flywheel 本体は特定ツールに依存しない。
...
- 実体: **プラグインを導入した独立リポジトリ**。1 つの守備範囲（ドメイン）を担い、1 つ以上の**ポジション**（`positions/<domain>.md`）で定義される。役割はエージェントごとに様々（開発・調査・運用横断など）。
...
- エージェントの「職務記述書」（守備範囲＝ポジション）。担当ドメイン／ミッション／関心範囲／権限／関係／関連度判定／記憶参照から成る。
...
3. 計画: 関連記憶を前ロード → タスク探索・分解（人間承認: FR-13）
4. 実行: 接続ツールへ委譲（開発なら claude-harness 等。破壊的/不可逆操作は人間承認: FR-22）
5. 検証: テスト・レビュー・動作確認（人間が最終確認: FR-32）
