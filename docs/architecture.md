...
- **起動方式は `claude -p` に一本化（YAGNI）**:
  ...
  - 権限は **FR-22 の承認境界**に従う: ローカル編集・commit・**topic push・PR（draft 含む）作成は自律可**（外向きだが可逆）、**merge / 本番影響 / 削除 / 履歴破壊が承認ゲート対象**（§5.2 step 4・§6・[#24](https://github.com/masanami/claude-flywheel/issues/24)）。起動例の `--permission-mode acceptEdits` は編集の自動承認で、force-push 等の不可逆操作は対象 repo の `settings.json`（deny）でゲートする。
...
- **【意思決定の主体】委譲先エージェントが『ユーザー＝意思決定者』役を担う**: 自走委譲（headless）には**対話相手の人間がいない**。接続ツールの**対話前提スキル**（例: claude-harness の `/define-feature` は要件・クリティカル設計を対話で決める前提）が選択肢提示で人間の回答を待つと、**意思決定で停止して先へ進めない**。これは特定スキルの問題ではなく、「自律オーケストレーション（flywheel）」と「対話前提のサブツール」の接続様式という flywheel 全体の論点。原則を次に固定する:
  ...
  - **FR-22 ゲートは維持**: 真に破壊的・不可逆な操作（マージ / 本番影響 / 削除 / 履歴破壊）の承認境界は別途維持する（§6）。**topic push・PR 作成は可逆な外向き操作としてサイクル内で自律可**（[#24](https://github.com/masanami/claude-flywheel/issues/24)）。重大で不可逆な前提・判断は報告して可視化する。「停止しない（意思決定移譲）」と「不可逆操作をゲートする（FR-22）」は別レイヤー。
...
| 4 | 真に破壊的・不可逆な操作（マージ/公開/削除/本番影響/履歴破壊）※topic push・PR 作成は可逆な外向き操作で対象外・自律可（[#24](https://github.com/masanami/claude-flywheel/issues/24)） | 5.2-4 | FR-22 |
