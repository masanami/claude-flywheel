...
| [heartbeat-check.sh](../scripts/heartbeat-check.sh) | 拍動停止の検知（最終 `cycle_end` からの空白営業日数がしきい値超過なら未終了 `*_start`・対応不整合 `*_end` とともに警告。読み取り専用。[#83](https://github.com/masanami/claude-flywheel/issues/83)） |
...
| [ledger-index.rb](../scripts/ledger-index.rb) | 課題台帳から**索引を投影**して stdout に出す（TSV・10 列）。run-cycle 手順0 が全文ロードの代わりに読む。**ファイルを作らない**＝正本は台帳 1 つ（`sync-free`）。読み取り専用。経路表は [contracts/ledger-read-scope.tsv](../contracts/ledger-read-scope.tsv)。[#122](https://github.com/masanami/claude-flywheel/issues/122) |
