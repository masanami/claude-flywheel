...
| [approval-latency.md](./approval-latency.md) | 承認→着手のレイテンシ（[#160](https://github.com/masanami/claude-flywheel/issues/160)）: 経路1（人間の純粋な承認）と経路2（対話代行）を分離した実測（承認→前進 中央値 13.5 分／純粋な cron モデルでは中央値 81 分・最大 3.5 日）、board をトリガにしない判断、軽量パスと「1 周 1 journal」の非両立、one-shot cron の本数が決める最小間隔 15 分、真正性の author 検査が判別力を持っていない実測。**AO-01〜AO-04 は決定済み**（目標 15 分 p90／軽量パスは journal を書かない／事前承認はレイテンシを理由に導入しない／FR-32 は検出のみ）。実装は I-1〜I-6 へ分割 | 確定 |
...
| [ledger-index.rb](../scripts/ledger-index.rb) | 課題台帳から**索引を投影**して stdout に出す（TSV・11 列）。run-cycle 手順0 が全文ロードの代わりに読む。**ファイルを作らない**＝正本は台帳 1 つ（`sync-free`）。読み取り専用。経路表は [contracts/ledger-read-scope.tsv](../contracts/ledger-read-scope.tsv)。[#122](https://github.com/masanami/claude-flywheel/issues/122) |
| [ledger-deps.rb](../scripts/ledger-deps.rb) | 課題台帳の**依存グラフ**（分類欄の `依存`）を解決し、違反（自己参照・存在しない ID・**循環依存**）と**起動可能集合**（`着手中` かつ依存先がすべて `完了`）を出す。入力は `ledger-index.rb` の投影であり **markdown を読み直さない**（パーサを 3 本目にしない）。完了判定は台帳＋アーカイブ（`challenge-archive*.md`）の両面。run-cycle 手順2・手順3 が呼ぶ。読み取り専用。[#150](https://github.com/masanami/claude-flywheel/issues/150) |
