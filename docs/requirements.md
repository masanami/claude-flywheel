...
- **FR-43** サイクルの good / bad（効いた点・うまくいかなかった点）を**軽量に記録**できること（実行ループ内では評価・改修をせず信号を残すだけ）。
- **FR-44** 蓄積した信号から傾向（再発する失敗・再利用価値のある成功）を集計し、**ハーネスの改修を提案**できること。改修の適用は人間承認を必須とする（承認ポイント）。
