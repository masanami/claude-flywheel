...
- **FR-22** 影響が大きい/後戻りしにくい操作（例: マージ、外部公開、削除、外部送信、履歴破壊）は **実行前に人間承認を必須** とすること（承認ポイント）。「外向き」と「不可逆」は別軸で、**close / ブランチ削除で戻せる可逆な外向き操作（topic push・PR〔draft 含む〕作成）は本ゲートの対象外**（サイクル内で自律実行可。人間の判断は PR レビュー＆マージに集約）。
