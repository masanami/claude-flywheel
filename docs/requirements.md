...
| アクター | 役割 |
| --- | --- |
| 管理者（人間） | 目標を設定し、要所で承認・方向修正を行う |
| AI 社員 | 目標に対しタスクを探索・分解・実行・検証・学習する主体 |
...
- **FR-06** 整理した課題のうち、**自身のポジションに関係するものを自律的に選定（ピックアップ）** し、目標・タスクに落とし込めること。
...
- **OQ-01 管理基盤の選定**: 目標・タスク・進捗を **どこで管理するか**（GitHub Issue/PR 中心 / リポジトリ内 Markdown ファイル / 両方併用）が未決。
  - 論点: 自律実行との親和性、人間の可視性、履歴の追いやすさ、既存 harness スキル（create-ticket 等）との接続。
