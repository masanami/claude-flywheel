...
- **Claude Code プラグインとして配布**し、機械（スキル/テンプレ）と状態（課題台帳/positions/memory）を分離する。状態は利用先ワークスペースに置く。
...
- **OQ-01 管理基盤の選定**:
  - **課題台帳**は「利用先ワークスペース内の一箇所のファイル（人間が記述）＝正本」を当面の方針とする（§5）。課題ソースは差し替え可能とし、外部ドキュメント（Notion/Doc/Slack 等）からの取り込みは後続。分類タグ・ステータスは正本（内部）で管理する。
