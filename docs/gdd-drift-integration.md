...
| ファイル | 変更 |
| --- | --- |
| `skills/periodic-audit/SKILL.md` | **新規（汎用機構）**。ジョブ別しきい値判定 → 対象 repo 決定（事前フィルタなし）→ 読み取り専用強制つきで cwd＝クローンへ委譲 → 3値を保存した結果解釈 → 台帳への冪等な起票（マーカー検算つき）→ 報告 |
...
| 論点 | 反映内容 |
| --- | --- |
| **読み取り専用の強制**（安全側） | 委譲先の権限を対象クローンの設定とブリーフの禁止文だけに委ねると、対象 repo 由来の指示で書き込み・push・外部送信が起こりうる。**事後の `git status` は検知であって防止ではない**（送信は取り消せない）ため、スキル本体に §読み取り専用の強制（RO-1〜RO-5）を新設し、**確認できなければ監査を起動しない fail-closed** にした。事後確認は 2 層目の検知として残す |
