...
> §1〜§6 は決定に至った根拠（確証事実・制約・選択肢比較）であり、実装の正本ではない。**実装の正本は [periodic-audit SKILL.md](../skills/periodic-audit/SKILL.md)（汎用機構の手順）と [challenge-ledger-format.md](./challenge-ledger-format.md)（台帳フォーマット）**。
>
> **本ドキュメントの位置づけ（重要）**: 実装は **`periodic-audit`＝「宣言された監査ジョブを低頻度で回し、結果を課題台帳へ冪等に起票する汎用機構」** として一般化された。**GDD の意味ドリフト監査は、その機構に載る最初の 1 ジョブ**（`cadence.json` の `audit.jobs[]` に宣言する）にすぎない。本ドキュメントは**その利用例に固有の検討と経緯**を記録するものであり、**汎用機構そのものの仕様は含まない**（§8.2 に切り分けの正本を示す）。
...
| **B: 独立スキル＋締めジョブからのしきい値起動** | `reflect` と同型。新スキル（後の `periodic-audit`）を作り、start-day の締めジョブが `journal/index.jsonl` でしきい値判定して起動する | start-day 締めジョブ |
...
- **i** は reflect と同じ算式・同じ入力（`journal/index.jsonl`）で実装でき、**追加の状態ファイルが要らない**。cadence.json の監査ジョブ宣言に `every_n_cycles`（例: 30。1 周 90 分・1 日 6 周として概ね週 1 に相当）を足すだけで済み、reflect との一貫性も保てる。**ii の「週 1」は i でほぼ表現できる**ため、壁時計を持ち込む必要は薄い。
...
- 起動条件は `(T + 1) % N == 0`（`T` ＝ `journal/index.jsonl` の総行数、`T + 1` ＝ 当周の周回番号、`N` ＝ そのジョブの `every_n_cycles`）。`index.jsonl` が無ければスキップする。
- start-day 手順 1 の値検証規律に従い、`every_n_cycles` は正の整数、不正値は既定値 `30` へ補正して継続（`execution_mode` のような fail-closed 扱いにはしない＝隔離境界ではないため）。
- **`audit` ブロック自体が無い（または `jobs` が空）場合は「定期監査を行わない」を既定とする**（opt-in。監査ジョブを宣言していないワークスペースで勝手にコストが発生しない）。この理由から**雛形 `templates/cadence.json` にはこのブロックを入れない**（雛形に入れると全ワークスペースで既定 ON になってしまう）。

## 7. 決定事項（2026-08-17・人間決定）

| ID | 問い | 決定 | 補足 |
| --- | --- | --- | --- |
| **Q-1** | 組み込み位置 | **案C: 独立スキル [periodic-audit](../skills/periodic-audit/SKILL.md) ＋ run-cycle 手順0 からのしきい値起動** | 案B（締めジョブ起動）は、締めジョブが `cycle.lock` の外で走り fan-out が翌周と重なりうるため不採用（§3.3） |
...
Q-3 を許した以上、**既存 ingestion の冪等性を壊さないこと**が絶対条件になる。契約は次の 4 点。正本は [challenge-ledger-format.md](./challenge-ledger-format.md) §監査元マーカー、手順は [periodic-audit SKILL.md](../skills/periodic-audit/SKILL.md) §5。
...
あわせて、実運用で発生した事故（**行番号演算によるエントリ移動が隣接エントリのマーカーを破壊**・2026-08-17 検出）を踏まえ、**台帳を機械で編集するすべての手順**に次を課した（[challenge-ledger-format.md](./challenge-ledger-format.md) §台帳を機械で編集するときの規律）: エントリ境界は `^### \[` の見出しパターンで切る／編集前後に「各エントリのマーカーが高々 1 つ・エントリ総数が期待値と一致」を `awk` で検算する／検算に失敗したらコミットせず元に戻す。この規律は periodic-audit だけでなく **§アーカイブのアトミック移動**からも参照させ、事故の再発経路を塞いだ（規律の詳細はレビュー反映で一時ファイル方式へ更新済み＝§8.1）。

### 7.2 harness へ戻すべき事項（Q-5・本 repo からは起票しない）

| # | 内容 |
| --- | --- |
| H-1 | `docs/gdd-design-draft.md` §3.2 は接続点を「run-cycle の観測ステップ（step 0）から低頻度で呼ぶ」と記載しているが、実装は**「step 0 はしきい値判定と起動のみ、実体は独立の汎用スキル periodic-audit」**（案C）になった。記述の更新可否を判断されたい |
...
| ファイル | 変更 |
| --- | --- |
| `skills/periodic-audit/SKILL.md` | **新規（汎用機構）**。ジョブ宣言の検証 → ジョブ別しきい値判定 → 対象 repo 決定（事前フィルタなし）→ **検証済みの実行境界の内側で** cwd＝クローンへ委譲 → **監査エンベロープ v1 の検証**と 3 値を保存した結果解釈 → 台帳への冪等な起票（マーカー検算つき）→ 報告 |
| `skills/run-cycle/SKILL.md` | 手順0 にジョブ別しきい値判定と periodic-audit 起動を追加（`--dry-run` パリティ・コミットは手順6・新規エントリは次周の手順1 へ）。手順6 のサイクルレポート項目に「定期監査の結果」を追加 |
| `skills/start-day/SKILL.md` | 手順1 に `audit` ブロックの既定（**未宣言＝OFF の opt-in**）と `audit.jobs[]` の値検証を追加（`id` の重複・`:` 混入は補正せず無効化＝マーカーの照合キーを壊さない） |
...
| `docs/architecture.md` | §4.1 のプラグイン構成に `periodic-audit/` を追加 |
| `.claude-plugin/plugin.json` | マイナーバージョンを 0.16.0 へ |

**`templates/cadence.json` は意図的に変更していない**。`audit` ブロックは opt-in（不在＝OFF）であり、雛形に含めると全ワークスペースで既定 ON になってしまうため（監査ジョブを宣言していないワークスペースで監査コストを発生させない）。
...
| 論点 | 反映内容 |
| --- | --- |
| **読み取り専用の強制**（安全側） | 委譲先の権限を対象クローンの設定とブリーフの禁止文だけに委ねると、対象 repo 由来の指示で書き込み・push・外部送信が起こりうる。**事後の `git status` は検知であって防止ではない**（送信は取り消せない）ため、スキル本体に読み取り専用の強制（RO-1〜RO-5）を新設し、**確認できなければ監査を起動しない fail-closed** にした。事後確認は 2 層目の検知として残す。**※この RO-1〜RO-5 は 4 巡目（§8.3）で「対象 repo の設定は境界にならない」として廃止され、§実行境界（B-1 / B-2）に置き換わっている** |
...
### 8.2 汎用機構への一般化（3 巡目・人間承認済み）

レビューで、**flywheel の配布面に GDD という特定方法論の語彙が入り込んでいる**ことが指摘された。機構としては正しく作れているが、名前と設定キーが GDD 固有になっていた。関心事を切り分けると:

| 要素 | flywheel の関心事か |
| --- | --- |
| 定期的に低頻度でジョブを回す（しきい値駆動・cadence 管理） | **はい**（reflect と同型） |
| 対象 repo のクローンへ接続ツールを委譲する | **はい**（run-cycle の中核） |
| 委譲先の結果を課題台帳へ冪等に起票する | **はい**（台帳は flywheel の正本） |
| 「保証台帳」「意味ドリフト」という**概念** | **いいえ**（GDD ＝ 接続ツール側の方法論） |

**機構は flywheel のもの、意味づけは接続ツールのもの**。この線でスキルを汎用機構へ再定義し、**GDD の意味ドリフト監査はその最初の利用例に降格**した:

| 面 | 一般化前 | 一般化後 |
| --- | --- | --- |
| スキル名 | `audit-drift` | **`periodic-audit`** |
| cadence キー | `drift: { every_n_cycles }` | **`audit: { jobs: [{ id, title, skill, every_n_cycles }] }`**（**何を監査するかは `skill` が宣言し、flywheel は解釈しない**） |
| 台帳マーカー | `audit:drift:<repo名>` | **`audit:<ジョブid>:<repo名>`**（同じ repo に複数種の監査を起票できる） |
| 起票者 | `drift 監査（機械生成）` | **`定期監査（機械生成）`** |
| 最小契約 AC-1〜AC-4 | 「接続ツールに要求する契約」 | **「監査スキル一般が満たすべき契約」**（位置づけのみ変更・内容は同一） |
| GDD 固有の記述 | スキル本文に散在 | **§4.3 の実装例（対応表）と本ドキュメント**に限定 |

- **複数ジョブを許す設計にした**。理由: (i) 台帳マーカーに**ジョブ識別子が必要**（同じ repo に複数種の監査を起票できる形にするため）なので、ジョブという概念はいずれにせよ要る。単一ジョブだと識別子が正当化しにくい定数になる。(ii) 性質の違う監査は**自然な頻度が違う**（ドリフト監査は週 1、依存脆弱性は日次など）ため、単一の `every_n_cycles` を共有すると早々に破綻する。(iii) 単一ジョブとの差は配列 1 段だけで、ジョブごとの処理は同一。
- **GDD 固有の知識は flywheel の配布面から消えた**。`periodic-audit` はフェーズ判定・保証台帳・意味ドリフトのいずれの概念も持たず、`skill` 文字列をそのまま委譲先へ渡すだけになっている。

### 8.3 レビュー反映（4 巡目）

| 論点 | 反映内容 |
| --- | --- |
| **実行境界**（安全側・最優先） | **対象 repo の `.claude/settings.json` を安全境界として扱わない**ことを明記し、旧 RO-1〜RO-5 を廃止。委譲してよいのは **B-1 親管理の固定実行ラッパー**か **B-2 専用の隔離コンテナ**（対象の作業コピーだけを mount し、他エージェントのワークスペース・`~/.claude.json`・SSH 鍵を共有しない／egress 遮断）を**検証できた場合のみ**とし、**検証できなければ委譲しない**（fail-closed）。`--permission-mode` / `--disallowedTools` と事後の `git status` は**多層防御の補助・2 層目の検知**であって境界ではない、と位置づけを明記 |
| **交換形式**（AC-1〜4） | 「フィールド名・構造・語彙を契約に含めない」は行き過ぎだった（任意形式からは AC-1〜4 を安定抽出できず、解釈の揺れが誤起票・指紋変動・更新スキップを生む）。**ツール非依存 ≠ 形式非依存**と整理し、**flywheel 側が中立の「監査エンベロープ v1」を定義**（`flywheel_audit` / `job_id` / `premise` / `stages[]` / `human_review`）。**ジョブ宣言の `adapter`** で既存ツールの出力をエンベロープへ変換させる**併用**とし、**エンベロープ検証 V-1〜V-7 に 1 つでも失敗したら起票しない**（監査不能へ合流）。`afp` の入力も検証済みエンベロープに限定 |
| **`skill` の永続化** | `skill` は引数を含む任意文字列であり、台帳・journal・レポート・`runs.jsonl` へ残すと露出面が広がる。**永続化面に出すのは `skill_label`（未宣言なら `id`）だけ**とし、`delegate_start --skill` にも `skill_label` を渡す。あわせて **`.flywheel/cadence.json` 自体が Git 追跡対象**である事実を明記し、「そもそも `skill` に秘密情報を書かない」を前提として置いた |
| **検証の一元化と再実行** | 検証規則の**正本を periodic-audit §ジョブ宣言の検証**に集約し、**`audit` ブロックを読むすべての経路（periodic-audit 手順1・run-cycle 手順0・start-day 手順1）が委譲前に同じ検証を fail-closed で実行**する（start-day を経由しない起動経路があるため）。**不正値は補正せずジョブ単位で無効化**（`every_n_cycles` も含む。台帳へ書き込む処理を駆動する設定のため安全側） |
| **`id` の許可文字** | `^[A-Za-z0-9._-]{1,64}$` に固定。改行・制御文字・`-->` がマーカーの境界を壊すため、**エスケープではなく許可文字リストで弾く**。同じ規則を challenge-ledger-format・start-day・run-cycle で参照 |
| **2 ファイル更新の原子性** | `mv` は 1 ファイルでは原子的だが **2 回の `mv` は原子的でない**。**トランザクションマーカー（`.flywheel/ledger-tx.json`）＋起動時復旧**を追加し、**追記側（アーカイブ）を先に置換**する順序を必須化（途中停止で残るのを「消失」ではなく検出可能な「重複」にする）。復旧表で 4 状態を分岐し、**どちらにも無い場合は台帳を触らず停止して人間へ報告** |
| **設定例の陳腐化** | `docs/gdd-drift-integration.md` §6.3 の設定例が旧 `drift` ブロックのままだった（そのまま設定すると監査は未宣言としてスキップされる）。`audit.jobs[]` 形式へ更新 |

**本 PR のスコープ外として残した項目（別課題の提案）**:

| # | 内容 | 本 PR で対応しない理由 |
| --- | --- | --- |
| S-1 | **B-1（親管理の固定実行ラッパー）の実装** | `scripts/` への実装追加であり、散文の設計変更である本 PR とは種類が違う。実装なしに「ラッパー経由で安全」と書くほうが危険なため、本 PR では**要件と fail-closed 挙動の定義まで**に留めた |
| S-2 | **`templates/container/compose.yml` を B-2 の要件へ是正**（`$HOME` 全体 mount をやめ、必要ファイルの個別 mount ＋ egress 制御へ） | 現行の `$HOME` 全体 mount は Issue #54 で**意図して選ばれたトレードオフ**であり（同ファイルのコメントに「エージェント間の隔離は提供しない」と明記済み）、変更には Dockerfile 側の `$HOME` 事前作成・所有権調整が伴う。**本 PR で片手間に触ると container モード全体の運用を壊しうる**ため分離した。本 PR では代わりに「**現行 compose は B-2 を満たさない**」と明記し、「コンテナなら安全」と読める記述を削除した |

**現時点の帰結（正直な状態）**: B-1 が未実装で、既定の container 構成が B-2 を満たさないため、**periodic-audit は現状どの repo にも委譲せず「境界未整備」を報告する**のが正しい動作になる。機構・契約・台帳規約は完成しているが、**実際に監査を回すには S-1 か S-2 のいずれかが先に要る**。
