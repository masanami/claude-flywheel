# エージェントmemory 運用方針

> ドメイン担当エージェントが、**ドメイン地図だけでなく暗黙知・経験**を蓄積し、横断的に自律して動けるようにするための記憶の運用を定義する。
> ブートストラップ（[bootstrap-domain-map](../skills/bootstrap-domain-map/SKILL.md)）で初期 seed を生成し、業務遂行のたびに更新していく。
> 記憶の読み書き（save / recall / promote / maintain）は [agent-memory](../skills/agent-memory/SKILL.md) スキルに集約し、bootstrap-domain-map・run-cycle はこれに委譲する。
>
> - ステータス: ドラフト（検討中）
> - 関連: [requirements.md FR-40〜42](requirements.md)（学習フェーズ）, [challenges.md 課題2](challenges.md), [self-improvement.md](self-improvement.md)（experience を内省ループの入力に使う）

## 1. なぜ memory か

- 課題2の本質は「**人間が把握しきれていない業務フロー・サービス関係を、エージェントが知識と経験として持つ**」こと。
- コードに**書かれていない**暗黙知（業務ルール・命名の意図・落とし穴）や、**過去の試行から得た経験**（何が効いた/失敗した）は、毎回ゼロから読み解くとコストが高い。
- → これらを永続化し、レビュー・実装の前提知識として供給する。これは**課題1ボトルネックB**（前提知識の不足）の解にも直結する。

## 2. 記憶の種類（type）

| type | 内容 | 例 | 主な生成元 |
| --- | --- | --- | --- |
| `map` | ドメイン地図 | サービス一覧、サービス間呼び出し/イベント、ドメイン境界 | ブートストラップ・コード解析 |
| `tacit` | 暗黙知 | 業務ルール、命名の意図、設計上の制約、注意点 | コメント/README/人間からの確認 |
| `experience` | 経験 | 過去タスクの結果、効いた手順・失敗、レビュー指摘の傾向 | 実行・検証後の学習 |
| `reference` | 参照 | API 契約、スキーマ、外部ドキュメント・ダッシュボードの場所 | ブートストラップ・運用 |

> `map` は対象リポジトリを **`repos.tsv` の `<name>`** で参照する（実体は `.flywheel/repos/` の作業用クローン。管理は [architecture.md §3.9.1](architecture.md)）。秘密情報はマニフェスト・memory に書かない。

### 2.1 experience の追加フィールド（自己改善ループ用）

`experience` は、自己改善（内省）ループ（[self-improvement.md](self-improvement.md)）の入力も兼ねる。run-cycle の学習ステップが **append のみ**で good/bad を記録し、傾向の集計と改修提案は別スキル [reflect](../skills/reflect/SKILL.md) が低頻度で行う。

```yaml
metadata:
  type: experience
  outcome: good | bad                 # この経験の評価
  target: skill:<name> | subagent:<name> | brief | position | recall | other  # どの資産の改善に効くか
  signal: <何が効いた/詰まったか 1行>
  recurrence: <同種が何回目か（任意）>
  confidence: high | medium | low     # 他の type と同じく必須（格納規約）
```

- **bad** = 改修トリガー、**good** = 再利用資産化・回帰ガード・recall 正例（[self-improvement.md §2](self-improvement.md)）。
- run-cycle 内では評価・改修をせず、good/bad を記録するだけ（軽量・冪等）。

## 3. 置き場所・構造

- **ドメインごとに記憶ストアを分ける**（横断知識は関係先ドメインへ相互リンク）。

```text
memory/
  <domain>/
    INDEX.md                  # そのドメインの記憶インデックス（1行/件）
    map-<slug>.md             # ドメイン地図の断片
    tacit-<slug>.md           # 暗黙知1件
    experience-<slug>.md      # 経験1件
    reference-<slug>.md       # 参照1件
```

> 構造は既存ハーネスのファイルベース memory（frontmatter + INDEX）の規約に揃える。配置場所の最終決定は [requirements.md OQ-01](requirements.md) と合わせて確定する。

### ファイル frontmatter（案）

```markdown
---
name: <short-kebab-case-slug>
description: <one-line。recall 時の関連判定に使う>
domain: <ドメイン名>
metadata:
  type: map | tacit | experience | reference
  sources: [<根拠となるファイル/URL/会話。tacit は確証方法の明記が必須。例: 実コード確証（services/billing/src/api.ts:42）、ドキュメント・推論由来（要検証）>]
  confidence: high | medium | low   # どれくらい確信しているか（特に tacit）
---

<事実・知見の本文。関連記憶は [[name]] でリンク>
```

- 1 ファイル = 1 事実（粒度を小さく保ち、recall と更新を容易に）。
- `confidence`（どれくらい確信しているか）とは**独立の軸**として、`sources` に**確証方法（provenance）**＝「実コード確証（file:path）」／「ドキュメント・推論由来（要検証）」を明記し、暗黙知の誤りを管理する（規約の詳細は [agent-memory スキル](../skills/agent-memory/SKILL.md)の「tacit の sources 規約」）。`confidence` だけで確定/未確証を判定しない。

## 4. ライフサイクル

*図: 記憶のライフサイクル — ブートストラップで初期記憶を作り、実行・検証で継続更新し、レビュー前に事前ロードして実装/レビュー品質を上げる。*

```mermaid
flowchart LR
    init["初期記憶<br/>（ブートストラップ: map/reference を seed・tacit 候補を抽出）"]
    grow["継続的に蓄積・更新<br/>（実行・検証: experience 追記・新発見の tacit 追記）"]
    out["質の高いレビュー/実装<br/>（レビュー前に関連 memory を事前ロード＝前提知識として供給）"]
    init --> grow --> out
```

- **生成**: ブートストラップで `map` / `reference` を seed、`tacit` 候補を抽出（人間確認は承認ポイント）。
- **更新（学習）**: タスクの実行・検証後に `experience` を追記（FR-40/41）。新たに判明した暗黙知も追記。
- **活用**: 要件・設計レビューや実装の**前ロード知識**として参照（課題1ボトルネックBの解）。
- **保守**: 人間が閲覧・修正・削除できる（FR-42）。未確証の判定は 2 条件で、それぞれ独立に効く: (1) `confidence: low` は確信度が低い＝未確証として扱う。(2) `sources` の確証方法が「ドキュメント・推論由来（要検証）」の記憶は、**`confidence` の高低に関わらず**未確証（要検証）として扱う（確証方法は `confidence` とは独立の軸。詳細は agent-memory スキルの provenance 規約）。

### 参照モデル（CLAUDE.md ＋ recall）

- **ベースライン（自動）**: ポジション要約・記憶 INDEX・recall 手順を各エージェントの **`CLAUDE.md`**（セッション開始時に自動ロード）に置く。メインセッションが起動時に「自分は誰／何を知っているか／どう引くか」を把握できる。
- **詳細（明示・選択）**: 記憶本文は全ロードせず、`agent-memory` の **recall** で関連分だけ取得する（量・トークン・関連性のため）。
- **サブエージェント委譲時**: CLAUDE.md は引き継がれないため、前提知識はブリーフに明記して渡す。

## 5. 公式 Claude Code memory との使い分け

claude-flywheel の agent-memory は、Claude Code 標準のファイルベース memory（`~/.claude/projects/.../memory/` ＋ `MEMORY.md`）とは**目的もスコープも別物**。両者は併用する。

| 観点 | 公式 Claude Code memory | flywheel agent-memory |
| --- | --- | --- |
| 置き場所 | `~/.claude/projects/...`（ローカル・個人） | エージェントrepo `memory/`（Git追跡・共有） |
| スコープ | ユーザー/プロジェクトのセッション文脈 | エージェントのドメイン知識・経験（成果物） |
| type | user / feedback / project / reference | map / tacit / experience / reference |
| 共有・レビュー | 個人ローカル、レビュー対象外 | チーム共有・Git レビュー・人間承認可 |
| ロード | セッション開始時に自動 | run-cycle / レビュー前に明示ロード（recall） |

**使い分けの指針**:
- **ドメイン知識・経験（成果物）** → flywheel agent-memory（repo 内）。共有・レビュー・version 管理したいため、公式の `~/.claude` には置かない。
- **作業上の好み・このユーザー/repo 特有の文脈** → 公式 memory に任せる。
- flywheel はファイル規約（frontmatter / INDEX）を公式に寄せるが、**ストアは別（in-repo）**。

## 6. 検討中の論点（Open Questions）

- 配置場所: リポジトリ内（Git 履歴で追える）か、別ストアか（[OQ-01](requirements.md)）。
- 横断知識の共有: 複数ドメインで共有する事実をどう持つ（共有領域 or 相互リンク）。
- 暗黙知の検証フロー: 未確証の推定（`confidence: low` や `sources` の要検証マーカー付き）を誰が・いつ確証へ昇格させるか（agent-memory の promote。昇格時は確証方法マーカーも実態に合わせて更新する）。
- recall の選定精度: タスク着手時にどの記憶をロードするかの判定ロジック（agent-memory の recall）の改善。
