...
| **版の値** | **そのテンプレートを最後に変更したときのプラグイン semver**（`.claude-plugin/plugin.json` の `version`）。**`<major>.<minor>.<patch>` の 3 要素ちょうど、かつ各要素が正規形（`0` または `[1-9]\d*`）のものだけを受理**し、それ以外（`0.20` / `0.20.0.1` / `00.20.0` / `01.02.03` / `v0.20.0` / prerelease）は形式不正として報告する | 人間が読める・順序が付く・リリースと対応する。内容ハッシュは順序が付かず、空白の変更でも動く。**受理を緩めると、形式不正であるべきマーカーが「追従済み」として黙殺される**——2 つの軸がある: ①要素数を可変にして欠けた分を 0 で補うと `@0.20` が `@0.20.0` と一致する ②先頭ゼロを許すと `to_i` が正規化して `@00.20.0` が `@0.20.0` と一致する（`@01.02.03` は `@1.2.3` に化けて「新しい」と誤報する）。いずれも PR #119 レビュー指摘 |
...
| 5 | マーカーの形が壊れている（`@` 無し・3 要素の semver でない・要素が正規形でない） | 「版マーカーの形式が不正」＋ 内容は状態 4 と同じ扱い | なし |
...
状態 5（形式不正）の状態空間は、**要素数**と**要素の正規形**の 2 軸で尽くす。片方だけを
洗うと取りこぼす——実際、要素数の軸だけを塞いだ最初の修正では `00.20.0` が 3 要素として
検査を通り、`to_i` の正規化で `0.20.0` と一致して黙殺された（PR #119 再レビュー指摘）。

| 軸 | 不正な例 | 受理し続ける例 |
| --- | --- | --- |
| 要素数 | `0.20` / `0.20.0.1` | `0.20.0` |
| 要素の正規形 | `00.20.0` / `01.02.03` / `0.00.0` / `0.20.00` / `000.0.0` | `0.0.0` / `0.0.1` / `10.20.30` |
| 文字種 | `1.2.3a` / `0.20.x` / `０.20.0`（全角） / `v0.20.0` / `latest` | — |
| 前後空白 | `@ 0.20.0`（マーカーの形の段階で不正） | — |
