...
| **版の値** | **そのテンプレートを最後に変更したときのプラグイン semver**（`.claude-plugin/plugin.json` の `version`） | 人間が読める・順序が付く・リリースと対応する。内容ハッシュは順序が付かず、空白の変更でも動く |
...
| 5 | マーカーの形が壊れている（`@` 無し・semver でない） | 「版マーカーの形式が不正」＋ 内容は状態 4 と同じ扱い | なし |
...
さらに両者を協調させる: マーカーが古い `CLAUDE.md` について内容チェックが**追従済み**を示す
場合、報告に「重要項目（§意思決定の主体）は追従済み」と添える。既知の偽陽性を、人間が
「マーカー行だけ直せばよい」と判断できる形に変える。
