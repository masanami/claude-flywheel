...
| `other`（`priority-policy.md` の場合） | `priority-policy.md`（優先度方針。人間専用ステート） | **編集不可**（人間のみが切り替える運用ファイルのため diff 提案・upstream Issue いずれも対象外）。所見は内省レポートに記述するに留める（承認ゲート付き自動切り替えはスコープ外） |
