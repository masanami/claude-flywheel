...
> 「実行ループ（[run-cycle](../skills/run-cycle/SKILL.md)）」と「自己改善ループ（[reflect](../skills/reflect/SKILL.md)）」を**分離**し、run-cycle は good/bad の軽量な記録だけを担い、改修判断は reflect に切り出す。
>
> - ステータス: ドラフト（検討中）
...
- → run-cycle には「**good/bad の軽量な記録**（評価ではなく append のみ）」だけを置き、傾向の集計と改修提案は**低頻度の別ループ**で行う。
...
### 層1: run-cycle 内の「good/bad の記録」（軽量・改修しない）

run-cycle の学習ステップ（[step 5](../skills/run-cycle/SKILL.md)）で、**good/bad を append するだけ**。評価も改修もしない。既存の `experience` 記憶型を再利用し、frontmatter にフィールドを足す（運用は [agent-memory.md §2.1](agent-memory.md)）。

```yaml
metadata:
  type: experience
  outcome: good | bad           # この経験の評価
  target: skill:<name> | subagent:<name> | brief | position | recall | other
...
run-cycle とは独立に起動し、蓄積した記録を集計してパターンを抽出 → 改修を**提案する**（自動では適用しない）。詳細手順は [reflect](../skills/reflect/SKILL.md)。
...
> この節は [reflect](../skills/reflect/SKILL.md) §3「分類（改修対象 = target）」の表と同内容のミラー。スキルは配布時にこの docs を実行時に参照できない前提のため、規約は両方に書く（[authoring-style.md](./authoring-style.md) 冒頭参照）。**表の行を追加・変更するときは両ファイルを同時に更新する**。

reflect が直接編集してよいのは、**エージェントrepo 側のローカル資産**だけ。**プラグイン本体の共通スキルは読み取り専用**なので直接編集せず、改善は **upstream（claude-flywheel）への Issue 起票**に倒す。**人間専用ステート（`priority-policy.md` 等）は、物理的にはエージェントrepo ローカル資産だが上記2分類のいずれにも属さない第3の扱い**（編集不可・diff 提案も upstream Issue も対象外）とする。
...
| `other`（`priority-policy.md` の場合） | `priority-policy.md`（優先度方針。切り替えの意思決定が人間に限られる運用ファイル） | **編集不可**（reflect は自律・低頻度実行でありその場の人間指示が無いため、切り替えの意思決定が人間に限られる本ファイルには diff 提案・upstream Issue いずれも対象外）。所見は内省レポートに記述するに留める（承認ゲート付き自動切り替えはスコープ外） |
...
*図: 2ループ — run-cycle が毎周 experience を蓄積し、reflect が低頻度で集計→改修提案。人間承認を経た資産更新が次サイクルへ還る弾み車。*

```mermaid
flowchart TD
    rc["run-cycle（毎周・軽量）<br/>実行→検証→学習"]
    rc -->|"good/bad を記録（append のみ）"| exp["experience<br/>(outcome / target / signal)"]
    exp -->|蓄積| reflect["reflect（低頻度・内省）<br/>集計→抽出→分類→提案→記録"]
    reflect --> diff["改修 diff 提案"]
    diff -->|"人間承認（承認ゲート）"| apply["ローカル資産を更新<br/>プラグイン改善は upstream Issue"]
    apply -->|"次の run-cycle が改善後の資産で回る（弾み車）"| rc
```
