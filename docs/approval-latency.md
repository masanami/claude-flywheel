...
> **ステータス**: ドラフト（設計メモ）。**論点①⑤は実コード・実測で確定、論点②③④に未決事項が残る**（§9 の AO-01〜AO-04）。
> 本メモは「承認から着手までの遅延をどこで縮めるか」を、実測・実コードの事実と、まだ人間が選ぶべき分岐に切り分けて記録する。実装は別 Issue（§8）。

FR-13（計画承認）は「タスク案を書く → 人間が `[x]` を入れる → **次の** run-cycle が検出して前進させる」という往復で、検出がサイクル境界に律速される。本メモは (1) その遅延を実測し、(2) 承認をイベントとして拾う経路の可否を実コードで判定し、(3) 残る設計分岐を選択肢として提示する。
...
| ② | サイクル境界より細かい軽量パス | **軽量パスを置く方向を推奨**。ただし「1 周 1 journal」と両立せず、**何を緩めるかは未決**（§5.4・AO-02） | 推奨（未決あり） |
| ③ | 事前承認の粒度 | **選択肢を提示するに留める**（§6）。採否は人間判断（AO-03） | **未決** |
| ④ | FR-32 も同時に扱うか | 実測では **FR-32 のほうが待ちが長い**（§2.4）。同時に扱う設計上の追加コストはほぼ無いが、採否は人間判断（AO-04） | **未決** |
...
| journal 1 周 1 ファイル ＋ `index.jsonl` 1 周 1 行 | **両立しない**（§5.4） | **要決定** |
...
> より短い間隔が要るなら `recurring: true` の cron 1 本に切り替える選択肢はあるが、`start-day` が one-shot を選んだ理由（**起動日以外に発火しない構造的保証**＝`--resume` で日付境界を跨いでも翌日に発火しない）を失う。recurring は 7 日で自動失効する別の寿命も持ち込む。**本メモは 15 分・one-shot 維持を推奨する。**

### 5.4 両立しない点 —「1 周 1 journal」（**未決 = AO-02**）
...
| 案 | 何を緩めるか | 契約物への波及 | 評価 |
| --- | --- | --- | --- |
| **A: 軽量パスは journal を書かない** | 「1 周 1 journal」の**「周」の定義をフルサイクルのみに狭める**。軽量パスの恒久記録は**台帳のコミット（Git 履歴）**が担い、次のフルサイクルの journal ①触った課題に事後記載する | **ゼロ**（スキーマ・バリデータ・board パーサ・reflect 判定すべて無変更） | **推奨**。#82 が「契約物の変更ゼロ」を採用根拠にしたのと同じ判断軸 |
| B: 軽量パスも journal 行を書き、reflect 側を直す | `index.jsonl` に周種別フィールドを足し、しきい値判定を「フルサイクル行のみ計数」へ変更 | **大きい**。`journal-index.schema.json` は `additionalProperties: false` のためスキーマ改訂が必須、board パーサの追随も必要 | 不採用推奨 |
| C: 軽量パスを作らず cadence 自体を 15 分へ | 何も緩めない | ゼロ | **不採用**（§3 の注記＝費用が約 6 倍） |

**案 A が緩めるもの（明示）**: 「run-cycle の 1 周は必ず 1 つの journal を残す」という現行の不変条件を、「**フルサイクルの 1 周は必ず 1 つの journal を残す。軽量パスは journal を残さず、Git 履歴と次のフルサイクルの journal が記録を担う**」へ弱める。これは**恒久記録の正本を journal から「journal ＋ Git 履歴」へ広げる**ことを意味し、本メモの権限では確定しない（AO-02）。
...
## 6. 論点③④ — 事前承認の粒度 / FR-32 の同時扱い（**未決**）

本メモは選択肢と推奨材料の提示に留める。**採否は人間の判断**（AO-03 / AO-04）。
...
**判断材料**: §3 の目標は**軽量パス単独で達成できる**ため、事前承認は「レイテンシ対策」としては不要である。導入するなら動機は別（人間の承認操作の**回数**を減らす＝滞留の文脈コスト削減）になる。**本メモは、レイテンシを理由に③を導入することは推奨しない。**
...
**推奨（弱）**: 検出は両方見る／**前進処理は FR-13 のみを軽量パスで行い、FR-32 の前進は従来どおりフルサイクルに委ねる**のが、軽量パスを最小に保つ切り方。ただし実測上の便益は FR-32 側が大きいため、**トランザクション復旧を軽量パスへ持ち込むコストを許容するかは人間判断**（AO-04）。
...
依存順。I-1 は本課題の採否と独立に単独マージできる。

| ID | 内容 | 前提 | 想定サイズ |
| --- | --- | --- | --- |
| **I-1** | **観測プレーンの条件① を board の現況へ追随させる**（`templates/runtime/README.md:5`・`docs/architecture.md:453` の 2 箇所）。「状態ファイルに書き込まない」→「エージェントの状態機械には書かない／人間の入力欄を人間 identity でコミットする経路は対象外」 | なし（**独立**） | S |
| **I-2** | **軽量パス（承認検出パス）のスキル化**。索引＋`git log` で FR-13 の `[x]` を検出しステータスを前進させる。前進対象が無ければ**ロックを取らずに終了**（§5.5）。journal は書かない（AO-02 の決定に従う） | AO-01・AO-02 の確定 | M |
| **I-3** | **`start-day` に軽量パスの cron 登録を追加**。間隔は cadence の新フィールド（例 `approval_check_interval_minutes`・既定 15）。**登録前のジョブ数見積もりに軽量パス分を加算**する（既存の上限判定規定を拡張。§5.3） | I-2 | M |
| **I-4** | **`noop-check.rb` と journal 規定の追随**。軽量パスが前進させた課題を次のフルサイクルの journal ①へ事後記載する規定と、軽量パス由来のイベントを「変化あり」に分類する判定 | I-2 | S |
| **I-5** | **承認の真正性を機械的検査へ格上げする**（§7.2）。エージェントのコミットと人間のコミットを判別可能にする（commit trailer / 専用 identity 等）＋構造不変条件テスト | なし（**独立**。③を検討するなら前提） | M |
| **I-6** | （AO-04 が「同時に扱う」の場合のみ）**FR-32 を軽量パスの対象に含める**。台帳トランザクション（`.flywheel/ledger-tx.json`）の復旧手順を軽量パスへ取り込む | I-2・AO-04 | M |

**テストの形**（`[prose-spec-invariant-tests]` に沿う）: I-2〜I-4 は散文規定になるため、①軽量パスとフルサイクルの**経路表の完全性**（どのステータス×チェック状態がどちらに割り当たるか）、②**否定検査**（軽量パスが journal・memory・`runs.jsonl` を書かないこと）、③**真理値表**（前進条件）を `scripts/tests/` の bash テストで固定する。

## 9. 未決事項（人間判断）

| ID | 未決事項 | 本メモの推奨 | 決めないと止まるもの |
| --- | --- | --- | --- |
| **AO-01** | 目標レイテンシの確定 | 業務時間内の承認は **15 分以内（p90）**／業務時間外は翌営業日（現状維持） | I-2・I-3 の間隔設計 |
| **AO-02** | §5.4「何を緩めるか」 | **案 A**（軽量パスは journal を書かない＝「1 周 1 journal」の「周」をフルサイクルに限定し、恒久記録を Git 履歴へ広げる） | I-2・I-4 |
| **AO-03** | 論点③ 事前承認の採否 | **レイテンシを理由には導入しない**（目標は軽量パス単独で達成できる） | I-5 の優先度 |
| **AO-04** | 論点④ FR-32 を同時に扱うか | 検出は両方／**前進は FR-13 のみ**を軽量パスで（弱い推奨）。実測便益は FR-32 側が大きく、トランザクション復旧のコスト許容が判断軸 | I-6 の要否 |
