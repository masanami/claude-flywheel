...
> **2026-09-13 追記（[#164](https://github.com/masanami/claude-flywheel/issues/164)）**: 本文中の「承認の真正性」（`[x]` が人間のコミットで入ったか＝`git log` の author 確認／対話代行の 2 経路）は**当時の規定**である。#164 で author 確認を廃止し、承認は台帳の `[x]`、回答は `人間の回答` の非空を正とする形に改めた（エージェントが書いてよいのは対話でその場の人間から得たときだけ）。本文の比較は当時の前提での評価として残す。
