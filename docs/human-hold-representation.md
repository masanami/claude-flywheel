...
> **これは判断材料であり、決定ではない。** [#116](https://github.com/masanami/claude-flywheel/issues/116) が挙げる 3 案（＋本ドキュメントで追加した 1 案）を、
> `claude-flywheel` と `claude-flywheel-board` の**実コードで波及先を確定**したうえで評価する。
> **案の選択は人間が行う**（ステータス語彙の変更は board への契約変更を伴うため）。選択後の実装計画は §6。
...
- **推奨は案 A（専用ステータス `人間対応待ち`）**。理由は「保留」が**課題の現在状態**であり、状態の正本はステータス行 1 箇所に置くのが本プロジェクトの不変則に合うため。加えて、案 A は**既存の承認ゲート（`計画承認待ち` ＋ `[x]`）と構造が同型**であり、新しい規律を発明しない（§4.1・§5）。
...
| (b') 未知フィールド＋ネスト継続行 | 同上 ＋ `  - 問い: …` / `  - session_id: …` | (b) と**完全に同じ**（継続行も無視され、他フィールドは無傷） |
...
```mermaid
stateDiagram-v2
    着手中 --> 保留: 親（または子）が人間にしか答えられない問いに達した
...
- `contracts/schemas/runs.schema.json` の `delegate_start` は `challenge` と `session_id` を**必須**にしている。
- `SKILL.md:113` 節の対応付け規律は「**同一 `session_id` の最新の未終了 start**」であり、課題 ID から未終了の委譲を引ける。
- したがって保留中の課題を再開するときは、`runs.jsonl` から当該 `challenge` の未終了 `delegate_start` を引いて `--resume` すればよい。

**帰結**: どの案も **`session_id` を台帳フィールドとして持つ必要はない**。台帳に持つと `runs.jsonl` と二重管理になり、不変則「正本 1 つ」に反する。
（注記: 保留が複数サイクルにまたがると `delegate_start` が未終了のまま残る。これは正常だが、拍動停止が疑われた周に
`heartbeat-check.sh` が「未終了の `*_start`」として列挙する対象になる——**放置された委譲と意図的な保留が区別できない**。
実害は警告レポートのノイズのみだが、案が決まったら `templates/runtime/README.md` の注記に 1 行足す価値がある。）
...
ただし案 D の観察は残る: **`delegate_end` の `result` に保留の事実を書くこと自体は、どの案とも両立し、推奨できる**（`result` は `minLength: 1` の自由文字列＝スキーマ変更なし）。未終了 `delegate_start` を残さずに閉じられ、`--resume` の宛先も §3.2 の規則で引ける。
...
**推奨: 案 A。** 決め手は「意味の正確さ」ではなく **fail-closed であること**と **`sync-free` であること**の 2 点。
...
| T13 | (ステータス, `人間の回答` の非空) の**真理値表**で、手順1 が前進させる／させないの対応が SKILL.md の記述と一致する | 真理値表 | 「回答が空でも前進する」「回答があっても止まる」の取り違え |
...
| **3** | claude-flywheel | `skills/run-cycle/SKILL.md`: 手順1 に「`人間対応待ち` かつ `人間の回答` 非空 → `着手中` へ前進」を追加／`:170` `:175` の保留規定を「ステータス → `人間対応待ち`」に／手順3 の対象条件と `--resume` の再開経路（§3.2 の `runs.jsonl` からの復元）を明記 | 同上。**この段から書き手が新ステータスを使い始める** |
...
**どの案を採るか。** 推奨は**案 A**（§4.5）。決定に必要な材料は §4 の比較表と §6 の実装計画。

副次的に、選択と同時に決めておくとよい点:
