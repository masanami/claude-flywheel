...
- **ドキュメント・推論由来（要検証）**: README・設計ドキュメント・会話・類推など、コード確証を経ていない場合。例: `ドキュメント・推論由来（要検証・README.md より推定）`。
...
- **`reflected` / `applied_as` はこの雛形に書かない**（reflect が後から付ける）。雛形をコピーして値を埋めると、日付でない値（`未処理` 等）が入り、その記憶が reflect の集計窓から外れる。
- bad は改修トリガー、good は再利用資産化・回帰ガード・recall 正例に使う。
- `reflected` / `applied_as` は **reflect スキルだけが付与**する（run-cycle の学習ステップは付けない）。reflect の集計窓の既定「未処理」は、**`reflected` に日付（`YYYY-MM-DD`）として解釈できる値があるか**で判定する（フィールドが無い・値が空・`未処理` のような語・プレースホルダのままは、いずれも未処理として拾われる）。
