...
> 設計の前提: 課題ソースは差し替え可能（pluggable）。記入形式・取り込み元マーカーは**本スキル内に直接記載**（手順3・4）。詳細な背景は plugin リポジトリの `${CLAUDE_PLUGIN_ROOT}/docs/architecture.md` §3.1 / `${CLAUDE_PLUGIN_ROOT}/docs/challenge-ledger-format.md`（実行には不要）。
...
外部の自由記述を、台帳の**人間記入欄**にマッピングする（形式は `${CLAUDE_PLUGIN_ROOT}/docs/challenge-ledger-format.md`）:
