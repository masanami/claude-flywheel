...
> **課題ソースは差し替え可能（pluggable・ソース非依存）**: 実体（共有 repo / Notion / Google Doc / Slack 等）は問わない。記入形式・取り込み元マーカーは本スキル内に直接記載（手順3・4）。本スキルが読み取り・正規化を担い、ソースを増やす／差し替えるときは `challenge-sources.md` に行を足すだけで済み、**それ以降の機械（run-cycle 以降）は変更不要**。
