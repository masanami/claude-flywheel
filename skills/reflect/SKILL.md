...
### 3. 分類（改修対象 = target）
改修対象を `target` 別に仕分ける。**スコープの線引き**が重要で、下表のとおり: エージェントrepo ローカル資産は直接 diff 提案、**プラグイン本体の共通スキルは編集不可 → upstream Issue** に倒す（背景は plugin の `${CLAUDE_PLUGIN_ROOT}/docs/self-improvement.md` §4）。
