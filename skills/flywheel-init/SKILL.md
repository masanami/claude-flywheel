...
- **exit code**: `0`＝追従済み（変更なし）／`--apply` で適用完了、`3`＝要移行（dry-run で変更が必要）、`1`＝検算に失敗（**1 バイトも書いていない**）、`2`＝検査不能（テンプレート/バリデータ不在・引数不正等）。`1` / `2` はサイクルレポート・報告に理由を残す（「変更なし」に読み替えない）。
...
- **承認（`[x]`）は機械が代筆しない**。旧形式では承認事実が見出し文言（`**タスク案（FR-13・承認済み …）**`）にしか無いことがあるが、移行は常に**未チェックで新設**し、旧見出しの文言を HTML コメントとして保全したうえで「人間判断が必要」へ列挙する。**出力のこの節は人間へそのまま伝える**（承認プロトコルの真正性。`docs/challenge-ledger-format.md` §承認プロトコル）。
