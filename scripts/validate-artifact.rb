...
# 全生成者（手書き＝記入例コピー / ingest-challenges）のエントリに共通して
# 存在する行だけを必須にする（受理されるべき正規形を拒否しないため。任意フィールド
# 〔完了条件・体感の緊急度・関連サービス・取り込み元〕は必須にしない）。
...
LEDGER_REF_FIELDS = [
  ...
  ["関連PR", /^- 関連PR:(.*)$/,
   %r{\A(?:[A-Za-z0-9._-]+/)?[A-Za-z0-9._-]+\#\d+\z},
   "`<owner>/<repo>#<番号>`（同一 owner なら `<repo>#<番号>`）のカンマ区切り"],
  # `依存`（先行課題。Issue #150 / FR-12）は**参照先が同じ台帳の課題 ID** である点だけが違い、
  # 「任意・複数可・カンマ区切り・空欄は該当なし（＝独立）」という値の規則は上の 3 種と同じ
  # ため、同じ枠組みで**形だけ**を検査する。ID の**接頭辞は固定しない**（`C-` を強制すると
  # 別の採番規則のワークスペースを弾く）——見出し `### [<ID>]` に書ける文字の集合として
  # `[A-Za-z0-9._-]+` を要求し、自由記述・URL（`/` を含む）・プレースホルダ（`（未作成）` 等）
  # を弾くところまでを担う。
  #
  # **意味（自己参照・存在しない ID・循環依存の検出と起動可能集合の算出）はここで行わない**:
  # それらは台帳とアーカイブ（`challenge-archive*.md`）を横断するグラフを要し、1 ファイルを
  # 受け取る本バリデータの責務ではない。正本は scripts/ledger-deps.rb（run-cycle 手順2・手順3
  # が呼ぶ）であり、**同じ規定を 2 つの実装に書かない**（docs/challenge-ledger-format.md
  # §検査の分担）。
  ["依存", /^- 依存:(.*)$/,
   %r{\A[A-Za-z0-9._-]+\z},
...
def check_ledger(file, expect_ids = nil, live: true, vocabulary:)
  ...
  entries.each do |lineno, heading, body|
    ...
    #     規律の awk 検算と同じ意味論: 値のある取り込み元マーカーはエントリごとに高々 1 つ）。
    t = body.count { |l| l =~ /^- 取り込み元: *[^ ]/ }
    if t > 1
      errors << "#{lineno}: マーカーの整合違反（取り込み元=#{t}。高々 1 つ）: #{heading[0, 60]}"
    end
    ...
    # (7) 参照フィールド（関連リポジトリ・関連Issue・関連PR）と `依存` の値の形。
    #     空欄は「該当なし」（`依存` では「独立」）で正規。
    #     (6) と同じく live（台帳）限定。
