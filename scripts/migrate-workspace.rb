...
CLASSIFY_FIELD_ORDER = [
  "担当ポジション", "関連サービス", "関連リポジトリ", "関連Issue", "関連PR", "依存",
...
# 台帳のみ補う任意フィールド（参照フィールドと `依存`。docs/challenge-ledger-format.md
# §関連リポジトリ・関連Issue・関連PR と §依存（先行課題）が「既存ワークスペースへの行追加は
# #88 の対象」と定める）。
# **アーカイブには追加しない**: いずれも run-cycle が計画時・PR 作成時に埋める欄であり、
# 完了済みエントリの履歴に空行を足しても埋まらない（原文保存の履歴を無駄に書き換えない）。
#
# **`依存` の空値は「独立」を意味する**（docs/challenge-ledger-format.md §依存（先行課題））。
# したがって全エントリへ空値で足しても既存の着手可能性は変わらない——**値の遡及記入は行わない**
# のに対し、フィールド行の追加は安全であり、テンプレート追従の対象として足す。
LEDGER_ONLY_INSERTS = [
  ["関連リポジトリ", ["- 関連リポジトリ:"]],
  ["関連Issue", ["- 関連Issue:"]],
  ["関連PR", ["- 関連PR:"]],
