...
#   - **ゲートごとの検査はゲート名のリストで駆動する**（(C) の GATES）。ゲート 1 つにつき
#     assert を手書きで並べると、4 つ目のゲートが増えたときに静かに漏れる。あわせて
#     **SKILL.md に実在するゲート名の集合がリストと一致すること**を照合し、リストに無い
#     ゲートが生えたらここで落ちるようにする（接続漏れの検査）。
...
# 一時停止プロトコルを共有する承認ゲート。**この 1 本のリストが以下の検査を駆動する**
# （ゲートを増やしたらここに 1 行足す。足さずに SKILL.md へ書くと下の集合照合が落ちる）。
# 形式: <ゲート名>:<ゲート規定が置かれる手順の節を保持する変数名>
GATES="FR-13:step2 FR-22:step4 FR-32:step4"

pause="$(line_with "$step2" '【一時停止と集合承認】承認ゲート（FR-13 / FR-22 / FR-32）に達したら')"
if [ -n "$pause" ]; then
  pass "(C) 手順2 に【一時停止と集合承認】の規定がある"
else
  fail "(C) 手順2 に【一時停止と集合承認】の規定がある"
fi
has "(C) 規定は FR-13 / FR-22 / FR-32 に同じ形で掛かる" 'FR-13（本手順）と FR-22 / FR-32（いずれも手順4）に**同じ形で**適用する' "$pause"
has "(C) サイクルを終了せず一時停止する" 'サイクルを終了せず一時停止し' "$pause"

# SKILL.md に実在する承認ゲート名の集合が GATES と一致すること（接続漏れの検査）。
declared="$(printf '%s\n' $GATES | cut -d: -f1 | sort -u | tr '\n' ' ')"
actual="$(grep -o '【承認ゲート FR-[0-9]*】' "$RUN_CYCLE" | sed 's/【承認ゲート //; s/】//' | sort -u | tr '\n' ' ')"
if [ "$declared" = "$actual" ]; then
  pass "(C) SKILL.md の承認ゲート名の集合が GATES と一致する（declared=${declared})"
else
  fail "(C) SKILL.md の承認ゲート名の集合が GATES と一致する" "declared=[${declared}] actual=[${actual}]"
fi
...
# ゲートごとの検査（GATES 駆動）。**3 ゲートが同一の一時停止プロトコルを共有する**ことを、
# ゲート名を列挙したリストから同じ 3 点で照合する。
for g in $GATES; do
  gate="${g%%:*}"; sec="${g##*:}"
  eval "secbody=\${$sec}"
  gline="$(line_with "$secbody" "**【承認ゲート ${gate}】**")"
  if [ -n "$gline" ]; then
    pass "(C) ${gate} のゲート規定が ${sec} にある"
  else
    fail "(C) ${gate} のゲート規定が ${sec} にある" "見つからない: **【承認ゲート ${gate}】**"
  fi
  has "(C) ${gate} は【一時停止と集合承認】のプロトコルを共有する" '【一時停止と集合承認】' "$gline"
  has "(C) 一時停止点の規定が ${gate} の停止位置を名指しする" "$gate" "$(line_with "$step2" '**一時停止点**')"
done
# FR-22 と FR-32 は同一の停止点（#176 の決定 1）。
has "(C) FR-22 と FR-32 は同一の停止点" '**FR-22 と FR-32 は同一の停止点**' "$(line_with "$step2" '**一時停止点**')"
has "(C) FR-32 は同じ周で完了＋即アーカイブまで進める" '同じ周で `完了確認待ち → 完了` へ進め、即アーカイブまで行う' "$(line_with "$step4" '**【承認ゲート FR-32】**')"
has "(C) FR-32 は同じ停止点で FR-22 も提示する" '**同じ停止点で下記【承認ゲート FR-22】も併せて提示する**' "$(line_with "$step4" '**【承認ゲート FR-32】**')"
has "(C) FR-13 は一時停止で承認を求める" '下記【一時停止と集合承認】で一時停止して対話で承認を求める' "$(line_with "$step2" '**【承認ゲート FR-13】**')"

# 終わり方は 1 組だけ（ゲートごとに第 2 のプロトコルが生えていない）。
n_all_endings="$(grep -cE '^    - \*\*\([a-z]\) ' "$RUN_CYCLE")"
if [ "$n_all_endings" = "3" ]; then
  pass "(C) 終わり方は SKILL.md 全体で 1 組だけ（ゲートごとに分岐していない）"
else
  fail "(C) 終わり方は SKILL.md 全体で 1 組だけ（ゲートごとに分岐していない）" "got=${n_all_endings}"
fi

# --- FR-22（#176）の決定を固定する
# 切り出しの開始・終了パターンに `\*` を使わない: awk -v は値のエスケープを解釈するため
# `\*\*` が `**` になり ERE として別物になる（既存の `\.` が偶然通っているのと同じ罠）。
fr22_4="$(section "$RUN_CYCLE" '同じ一時停止点で、' '^### 5')"
if [ -n "$fr22_4" ]; then
  pass "(C) 手順4 の FR-22 ブロックを切り出せた"
else
  fail "(C) 手順4 の FR-22 ブロックを切り出せた"
fi
has "(C) 決定2: FR-22 の集合は完了へ進めるエントリのうち未マージの昇格 PR を持つもの" \
  '**FR-22 の集合は、その周に `完了確認待ち → 完了` へ進めるエントリのうち未マージの昇格 PR を持つもの**' "$set_def_22"
has "(C) 決定2: 未マージかどうかは外部実状態で確かめる" '**未マージかどうかは外部実状態を読んで確かめる**' "$set_def_22"
has "(C) FR-22 のマージは前進＋アーカイブより先" '**昇格マージを先に実行し、マージ済みかを外部実状態で確認してから**' "$fr22_4"
has "(C) 決定5: 一時停止中に CI・外部レビューの完了を待たない" '**一時停止中に CI・外部レビューの完了を待たない**' "$fr22_4"
has "(C) 決定6: 即アーカイブの規定は変えない" '即アーカイブの規定は変えない' "$fr22_4"
has "(C) 決定6: 未マージの昇格 PR をレポートと journal ③ に出す" 'サイクルレポートと journal ③ に必ず出す' "$fr22_4"
has "(C) 手順6 のサイクルレポートに未マージの昇格 PR を出す" '**未マージのまま `完了` にした昇格 PR**' "$step6"
has "(C) 手順6 の journal ③ に未マージの昇格 PR を出す" '**FR-22 の承認がそろわず未マージのまま `完了` にした昇格 PR も、課題 ID を添えてここに必ず書く**' "$step6"
# 決定3: 承認チェックボックスを 3 つ目として足し、ステータス語彙は増やさない。
has "(C) 決定3: 台帳テンプレートに 3 つ目の承認チェックボックスがある" '  - [ ] 昇格を承認（FR-22）' "$(cat "$LEDGER_TPL")"
has "(C) 決定3: 台帳フォーマットに 3 つ目の承認チェックボックスがある" '  - [ ] 昇格を承認（FR-22）' "$(cat "$LEDGER_FMT")"
has "(C) 決定3: 承認の検出は 3 ラベルの前方一致" '**`計画を承認` / `完了を承認` / `昇格を承認` までの前方一致**' "$(cat "$LEDGER_FMT")"
lacks "(C) 決定3: ステータス語彙に昇格を足していない" '昇格' "$(cat "$VOCAB")"
...
# 3 つ目の承認チェックボックス（`昇格を承認`・FR-22 / #176）を足した台帳が、消費者を壊さない
# ことを実測する。**必須行にはしない**（新設の行を必須にすると既存ワークスペースの正規な
# 台帳を一斉に弾く）ため、「足しても受理される」「足さなくても受理される」の両方を置く。
"$RUBY" -i -pe 'sub(/^(  - \[[ xX]\] 完了を承認.*)$/) { "#{$1}\n  - [ ] 昇格を承認（FR-22）" }' "$tmp/ledger.md"
n_promote="$(grep -cF '  - [ ] 昇格を承認（FR-22）' "$tmp/ledger.md")"
if [ "$n_promote" -ge 1 ]; then
  pass "(E) 正例に 3 つ目の承認チェックボックスを足せた（${n_promote} 行）"
else
  fail "(E) 正例に 3 つ目の承認チェックボックスを足せた" "置換が 0 行（fixture の形が変わった）"
fi
if "$RUBY" scripts/validate-artifact.rb ledger "$tmp/ledger.md" >"$tmp/val3.out" 2>&1; then
  pass "(E) 3 つ目の承認チェックボックスがあっても validate-artifact.rb ledger が受理する"
else
  fail "(E) 3 つ目の承認チェックボックスがあっても validate-artifact.rb ledger が受理する" "$(head -3 "$tmp/val3.out")"
fi
idx3="$("$RUBY" scripts/ledger-index.rb "$tmp/ledger.md" 2>"$tmp/idx3.err")"
rc3=$?
if [ "$rc3" -eq 0 ]; then
  pass "(E) 3 つ目の承認チェックボックスがあっても ledger-index.rb が exit 0"
else
  fail "(E) 3 つ目の承認チェックボックスがあっても ledger-index.rb が exit 0" "exit=${rc3} $(cat "$tmp/idx3.err")"
fi
# approvals 列は 2 文字のまま（昇格は索引に載せない＝手順4 は完了確認待ちを全文で開く）。
w3="$(printf '%s\n' "$idx3" | tail -n +2 | awk -F'\t' -v c="${col:-0}" 'c > 0 { print length($c) }' | sort -u | tr '\n' ' ')"
if [ "$w3" = "2 " ]; then
  pass "(E) 昇格の承認は索引に載せない（approvals 列は 2 文字のまま）"
else
  fail "(E) 昇格の承認は索引に載せない（approvals 列は 2 文字のまま）" "幅=[${w3}]"
fi
