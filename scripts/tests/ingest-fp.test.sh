...
has "規定: 手順側に PR2（説明の要約化）より前という順序依存がある" "$MIGRATION_DOC" \
...
has "移行: PR2 の実施条件（移行完了まで禁止）が手順側にある" "$MIGRATION_DOC" \
...
has "SKILL: ①移行が要る条件が本体に残っている" "$SKILL_MD" "初回だけ移行が要る"
has "SKILL: ②発火の見分け方（全件一斉不一致）が本体に残っている" "$SKILL_MD" \
    '発火の見分け方は「マーカー付き全エントリが一斉に `fp` 不一致になる」'
has "SKILL: ②通常の外部更新と取り違えない注意が本体に残っている" "$SKILL_MD" \
    "1 件だけの不一致は通常の外部更新であって移行ではない"
