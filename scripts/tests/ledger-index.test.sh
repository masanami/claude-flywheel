...
# ===========================================================================
# T5b 否定検査: 要約形の説明でも索引に漏れない（#130 PR2）
#   上の T5 は引用行（`>`）と定型文字列 `原文引用` に依存している。PR2 で説明欄が要約に
#   なると引用行が無くなるため、**索引が要約本文をそのまま出力していても T5 は緑になる**
#   ＝「説明を索引に載せない」契約が新形では固定されていない。要約形専用の台帳で、
#   **`説明` ラベルと要約本文のどちらも出力に現れない**ことを固定する。
#   1 行要約と形 D（1 行要約＋ネスト箇条書き）の両方を置く——後者は複数行の許容で
#   入った形であり、ネスト項目が継続行として索引へ流れ込む経路がここにしか無い。
# ===========================================================================
SUMMARY_LEDGER="$tmp/challenge-ledger-summary.md"
cat > "$SUMMARY_LEDGER" <<'SUMMARY_EOF'
# 課題台帳（Challenge Ledger）
...
- 承認（人間がチェック）:
  - [ ] 計画を承認（FR-13）
  - [ ] 完了を承認（FR-32）
- 取り込み元: src / o/r1#9（取り込み: 2026-08-30）<!-- fp:2:9c56b5c0a92d -->
- 備考: 備考の本文
...
- 承認（人間がチェック）:
  - [x] 計画を承認（FR-13）
  - [ ] 完了を承認（FR-32）
- 取り込み元: src / o/r2#10（取り込み: 2026-08-30）<!-- fp:2:0123456789ab -->
- 備考: 備考の本文
SUMMARY_EOF

sum_out="$(run "$SUMMARY_LEDGER")"; sum_rc=$?
eq "T5b: 要約形の台帳で exit 0" "$sum_rc" "0"
eq "T5b: 2 件とも投影される（空振りの否定検査にしない）" \
   "$(printf '%s\n' "$sum_out" | tail -n +2 | cut -f1 | tr '\n' ',')" "C-201,C-202,"
hasnt "T5b: 説明ラベルが索引に現れない" "$sum_out" "説明"
hasnt "T5b: 1 行要約の本文が漏れていない" "$sum_out" "要約本文は索引に出てはならない"
hasnt "T5b: 形 D の 1 行要約が漏れていない" "$sum_out" "一行目の要約も索引に出てはならない"
hasnt "T5b: 形 D のネスト項目が漏れていない" "$sum_out" "ネストした内訳も索引に出てはならない"
