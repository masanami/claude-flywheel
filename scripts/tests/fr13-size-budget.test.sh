...
has "(C) 上限到達＝承認した規模を超えた、を続行前に報告する" "$step3" \
  '**上限到達は承認した規模（`予算上限`）を超えたことを意味する**ため、続行（`--resume`）や PR 作成へ進む前に'
has "(C) --resume にも同じ導出値を渡す（上限到達からの続行）" "$step3" '`--max-budget-usd` に同じ導出値を改めて指定'
has "(C) --resume にも同じ導出値を渡す（作業のやり直し・回答を渡す再開）" "$step3" \
  '作業のやり直し・回答を渡す再開は委譲時と同じ導出値を渡す'
