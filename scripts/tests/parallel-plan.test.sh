...
# FR-13 の承認ゲートを緩めない。#164 で承認も集合単位になった（そろうまで進めない）ため、
# 固定するのは「1 件だけ承認された時点では進めない」こと（規定の本体は pause-for-set-approval.test.sh）
has "(B) FR-13 は集合の承認がそろってから進める（ゲートを緩めない）" "${step2}" \
  '**そろった時点でまとめて `着手中` へ進める**（1 件だけ承認された時点では進めない'
