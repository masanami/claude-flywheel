...
BUDGET_FORMULA='当周の既消費額 ＋ 起動済み未合流分の予約額 ＋ これから起動する分の `--max-budget-usd` > `cycle_budget_usd`'
...
# --- 枠分割（#173）: 評価式への入り方。式そのもの（BUDGET_FORMULA）は変えずに、
#     どちらの枠がどの項へ入るかを規定が明示していること。
has "(B) 予約額と「これから起動する分」はその起動に実際に渡す値で数える" "${step3}" \
  '**「これから起動する分の `--max-budget-usd`」と「起動済み未合流分の予約額」は、どちらもその起動に実際に渡す値**'
has "(B) 実装の起動の判定だけは総枠（実装枠＋レビュー対応枠）で行う" "${step3}" \
  '**ただし実装の起動を評価するときだけは、`これから起動する分` を「実装枠 ＋ レビュー対応枠」＝その委譲の総枠として判定する**'
has "(B) 総枠を使うのは判定だけで予約額・フラグ値ではない（二重計上しない）" "${step3}" \
  '**総枠を使うのは起動可否の判定だけで、`--max-budget-usd` に渡す値でも予約額でもない**'
has "(B) 未起動のレビュー対応は予約額に入れない（予約額の意味を保つ）" "${step3}" \
  'まだ起動していないレビュー対応は走っていないため予約額に入れない'
# 否定検査: 既定値の根拠が「L の上限 100 の 3 件分」のまま取り残されていないこと（総枠は 140）
hasnt "(B) 否定検査: 既定 300 の根拠が旧注記（100 の 3 件分）のまま残っていない" "${step3}" \
  'L サイズの委譲上限 100 の 3 件分'
...
has "(B) 既消費額の定義がある（total_cost_usd の総和）" "${step3}" \
  '**合流済み**の各委譲の返り値 `total_cost_usd` の総和'
has "(B) --resume 分も既消費額に加える" "${step3}" '`--resume` した分も各起動分を加える'

# **複数件を同時に起動する周**（#149）: 返り値がまだ無い分を 0 と見なすと天井が起動本数分だけ
# 掛け算される。予約額の項が消えると式は逐次専用へ退行するため、定義・振り替え・退化を対で固定する。
has "(B) 予約額の定義がある（起動済み未合流分の --max-budget-usd の総和）" "${step3}" \
  '既に起動して**まだ合流していない**委譲の `--max-budget-usd` の総和'
has "(B) 予約額が無いと天井が起動本数分だけ掛け算されると説明されている" "${step3}" \
  '天井が起動本数分だけ掛け算される'
has "(B) 合流したら予約額から外し確定消費へ振り替える" "${step3}" \
  '**合流したらその委譲を予約額から外し、確定した `total_cost_usd` を既消費額へ振り替える**'
has "(B) 二重計上しない（予約額と既消費額）" "${step3}" '予約額と既消費額で二重に数えない'
has "(B) 逐次の周では従来の式に退化する（1 件ずつ起動する周の予約額は 0）" "${step3}" \
  '先行分が合流済みのため予約額が 0 になり、従来と同じ式に一致する'
